    ...
    <ScrollView style={styles.container}>
        ...
        <View style={styles.profileCard}>
          <View style={styles.avatar}>
            <Text style={styles.avatarText}>
              {user?.company?.[0] || user?.full_name?.[0] || user?.email?.[0] || '?'}
            </Text>
          </View>
          <View style={styles.profileInfo}>
            <Text style={styles.profileName}>{user?.full_name || user?.company || 'User'}</Text>
      ...
      {isAdmin && (
        <View style={styles.section}>
          ...
          >
            <Ionicons name="people-outline" size={22} color={colors.purpleBright} />
      ...
      <View style={styles.footer}>
        ...
        <Text style={styles.copyright}>© 2025 Sentinel Authority</Text>
      </View>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  profileName: { color: colors.textPrimary, fontSize: 16, fontWeight: '600' },
  profileEmail: { color: colors.textTertiary, fontSize: 13, marginTop: 2 },
  roleBadge: {
  ...
  roleText: { color: colors.purpleBright, fontSize: 10, fontWeight: '600', letterSpacing: 0.5 },
  menuItem: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: colors.bgCard,
  ...
  menuText: { color: colors.textSecondary, flex: 1, marginLeft: spacing.md, fontSize: 15 },
  logoutBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
  ...
  logoutText: { color: colors.redBright, fontWeight: '600', marginLeft: spacing.sm },
  footer: { alignItems: 'center', padding: spacing.xl, marginTop: spacing.md },
  version: { color: colors.textTertiary, fontSize: 12 },
  copyright: { color: colors.textTertiary, fontSize: 11, marginTop: spacing.xs },
});
