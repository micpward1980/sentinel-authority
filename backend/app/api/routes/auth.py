...
from app.models.models import Organization, User, UserSession
from app.services.audit_service import write_audit_log
from app.services.email_service import (
    notify_admin_new_registration,
    notify_registrant_pending,
...
def _hash_reset_token(token: str) -> str:
    """One-way hash for reset token storage — raw token is never persisted."""
...
async def verify_2fa_login(
    ...
    try:
        ...
        if payload.get("type") not in ("temp", "access"):
    ...
    await write_audit_log(
        db, action="user_login", resource_type="user", resource_id=user.id,
        user_id=user.id, user_email=user.email,
        details={"ip": request.client.host if request.client else "unknown", "2fa": True},
    )
    await db.commit()
    ...
    return {
        ...
        "user": {
            ...
            "role": user.role, "organization": user.organization, "organization_id": user.organization_id,
        },
    }
...
async def register(request: Request, user_data: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    await write_audit_log(
        ...
        user_id=user.id, user_email=user.email,
        details={"organization": user.organization, "ip": request.client.host if request.client else "unknown"},
    )
    await db.commit()
    ...
    await write_audit_log(
        db, action="user_login", resource_type="user", resource_id=user.id,
        user_id=user.id, user_email=user.email,
        details={"ip": request.client.host if request.client else "unknown", "2fa": False},
    )
    await db.commit()
...
async def login(request: Request, credentials: UserLogin, db: AsyncSession = Depends(get_db)):
    ...
    if not user or not verify_password(credentials.password, user.hashed_password):
        if user:
            ...
            await db.commit()
        await write_audit_log(
            db, action="login_failed", resource_type="user", user_email=credentials.email,
            details={"ip": request.client.host if request.client else "unknown", "reason": "invalid_credentials"},
        )
        await db.commit()
    ...
    # 2FA check — issue short-lived temp token with explicit type
...
async def logout(
    ...
    if sid:
        ...
        if session:
            ...
            await db.commit()
    await write_audit_log(
        db, action="user_logout", resource_type="user",
...
async def forgot_password(
    ...
    # Send raw token to user — it never touches the DB
...
async def reset_password(request: ResetPasswordRequest, db: AsyncSession = Depends(get_db)):
    # Hash the incoming token and query against the stored hash
...
async def verify_reset_token(token: str, db: AsyncSession = Depends(get_db)):
    """Verify if a reset token is valid. Returns only {valid: true} — never exposes email."""
    ...
    # Return only validity — do not expose email
    return {"valid": True}
...
async def update_profile(
    ...
    await db.refresh(user)
    await write_audit_log(
        db, action="profile_updated", resource_type="user", resource_id=user.id,
