  ...
  return (
    <div className="space-y-6">
      <SectionHeader label="Administration" title="User Management" description="Manage admin and applicant accounts"
        action={<button onClick={() => setShowInviteModal(true)} className="sexy-btn px-4 py-2 flex items-center gap-2" style={{background: styles.purplePrimary, border: '1px solid ' + styles.purpleBright, color: 'rgba(255,255,255,.94)'}}><Plus className="w-4 h-4" /> Invite User</button>}
      />
      ...
      <Panel>
        <div className="flex items-center gap-3">
          ...
          <input type="text" placeholder="Search by name, email, or company..." value={search} onChange={(e) => setSearch(e.target.value)} className="sexy-input flex-1 px-4 py-3" style={{background: 'rgba(255,255,255,0.03)', border: '1px solid ' + styles.borderGlass, color: styles.textPrimary, outline: 'none'}} />
        </div>
      </Panel>
      <Panel>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {user.role === 'pending' && (
                    <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                      <button onClick={() => handleApproveUser(user.id, user.email)} className="px-2 py-1 text-xs" style={{background: 'rgba(34,197,94,0.15)', color: '#22c55e', border: '1px solid rgba(34,197,94,0.3)', cursor: 'pointer'}}>Approve</button>
                      <button onClick={() => handleRejectUser(user.id, user.email)} className="px-2 py-1 text-xs" style={{background: 'transparent', color: '#d65c5c', border: '1px solid rgba(255,255,255,0.07)', cursor: 'pointer'}}>Reject</button>
                    </div>
                  )}
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50" style={{background: 'rgba(0,0,0,0.7)'}}>
          <div className="w-full max-w-md mx-4 p-6" style={{background: styles.bgDeep, border: '1px solid ' + styles.borderGlass}}>
            ...
            <div className="mt-6 space-y-3">
              <button onClick={handleInvite} disabled={inviteLoading} className="sexy-btn w-full px-4 py-3" style={{background: styles.purplePrimary, border: '1px solid ' + styles.purpleBright, color: 'rgba(255,255,255,.94)', opacity: inviteLoading ? 0.7 : 1}}>{inviteLoading ? 'Creating...' : 'Create Account'}</button>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50" style={{background: 'rgba(0,0,0,0.7)'}}>
          <div className="w-full max-w-md mx-4 p-6" style={{background: styles.bgDeep, border: '1px solid ' + styles.borderGlass}}>
            ...
            <div className="space-y-2 mb-6">
              ...
              <button onClick={() => handleDeleteUser(selectedUser.id, selectedUser.email)} className="w-full px-4 py-3 flex items-center gap-3" style={{background: 'transparent', border: '1px solid rgba(255,255,255,0.07)', color: styles.accentRed, textAlign: 'left'}}><X className="w-4 h-4" /> Delete User</button>
            </div>
