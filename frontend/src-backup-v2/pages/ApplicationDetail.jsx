    ...
    <div className="space-y-6">
      {emailPreview && (
        <div style={{position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)'}}>
          <div style={{background: styles.bgDeep, border: '1px solid ' + styles.borderGlass, borderRadius: '18px', maxWidth: 'min(650px, 95vw)', width: '95%', maxHeight: '80vh', overflow: 'hidden', display: 'flex', flexDirection: 'column'}}>
            ...
            <div style={{padding: '16px 24px', borderTop: '1px solid ' + styles.borderGlass, display: 'flex', justifyContent: 'flex-end', gap: '10px'}}>
              <button onClick={() => setEmailPreview(null)} style={{padding: '8px 20px', background: 'transparent', border: '1px solid ' + styles.borderGlass, color: styles.textSecondary, fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>Cancel</button>
              <button onClick={confirmStateChange} className="sexy-btn" style={{padding: '8px 20px', background: emailPreview.newState === 'suspended' ? 'rgba(214,92,92,0.2)' : styles.purplePrimary, border: '1px solid ' + (emailPreview.newState === 'suspended' ? 'rgba(214,92,92,0.4)' : styles.purpleBright), color: emailPreview.newState === 'suspended' ? '#D65C5C' : '#fff', fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
        ...
        <div style={{display: 'flex', gap: '12px'}}>
          {app.state === 'pending' && (
            <button onClick={handleAdvanceToReview} className="px-4 py-2 transition-all" style={{background: 'transparent', border: '1px solid rgba(255,255,255,.10)', color: styles.accentAmber, fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
              Begin Review
            </button>
          )}
          {(app.state === 'pending' || app.state === 'under_review') && (
            <button onClick={handleApprove} className="px-4 py-2 transition-all" style={{background: 'transparent', border: '1px solid rgba(255,255,255,.10)', color: styles.accentGreen, fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
              Approve Application
            </button>
          )}
          {app.state === 'approved' && (
            <button onClick={handleScheduleTest} disabled={scheduling} className="px-4 py-2 transition-all" style={{background: styles.purplePrimary, border: `1px solid ${styles.purpleBright}`, color: 'rgba(255,255,255,.94)', fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: scheduling ? 'wait' : 'pointer', opacity: scheduling ? 0.7 : 1}}>
              {scheduling ? 'Scheduling...' : 'Schedule CAT-72 Test'}
            </button>
          )}
          {['pending','under_review','approved','testing','conformant'].includes(app.state) && (
            <button onClick={handleSuspend} className="px-4 py-2 transition-all" style={{background: 'transparent', border: '1px solid rgba(255,255,255,0.07)', color: '#D65C5C', fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
              Suspend
            </button>
          )}
          {(app.state === 'suspended' || app.state === 'revoked') && (
            <button onClick={handleReinstate} className="px-4 py-2 transition-all" style={{background: 'transparent', border: '1px solid rgba(255,255,255,.10)', color: styles.accentGreen, fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
              Reinstate
            </button>
          )}
          {app.state === 'expired' && (
            <button onClick={handleReinstate} className="px-4 py-2 transition-all" style={{background: 'transparent', border: `1px solid ${styles.purpleBright}`, color: styles.purpleBright, fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
              Re-open
            </button>
          )}
          <button onClick={handleDeleteApplication} className="px-4 py-2 transition-all" style={{background: 'transparent', border: '1px solid rgba(255,255,255,0.07)', color: '#D65C5C', fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
            Delete
          </button>
        </div>
        )}
      ...
      <div>
        ...
        <h1 style={{fontFamily: "'Source Serif 4', serif", fontSize: 'clamp(24px, 5vw, 36px)', fontWeight: 200, margin: 0}}>{app.system_name}</h1>
      ...
      if (newState === 'approved') {
        try {
          ...
          console.log('Auto-provision note:', provErr.message);
              ...
              className="px-3 py-2"
              style={{background: 'transparent', border: `1px solid ${styles.borderGlass}`, color: styles.textPrimary, fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px'}}
            >
              ...
              <div key={c.id} style={{padding: '12px 0', borderBottom: `1px solid rgba(255,255,255,0.04)`}}>
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px', marginBottom: '6px'}}>
                  <div style={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                    ...
                    {c.user_role === 'admin' && <span style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '9px', padding: '2px 6px', background: 'transparent', color: styles.purpleBright, textTransform: 'uppercase', letterSpacing: '0.5px'}}>Admin</span>}
                    {c.is_internal && <span style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '9px', padding: '2px 6px', background: 'transparent', color: styles.accentAmber, textTransform: 'uppercase', letterSpacing: '0.5px'}}>Internal</span>}
                  </div>
                  <div style={{display: 'flex', alignItems: 'center', gap: '8px'}}>
