    ...
    <div className="space-y-6" style={{maxWidth: "1000px", margin: "0 auto"}}>
      ...
      <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px'}}>
        <div>
          ...
          <h1 style={{fontFamily: "'Source Serif 4', serif", fontSize: 'clamp(24px, 5vw, 36px)', fontWeight: 200, margin: 0}}>Welcome{user?.full_name ? `, ${user.full_name.split(' ')[0]}` : ''}</h1>
      ...
      <Panel>
        ...
        {applications.length === 0 ? (
          <div style={{textAlign: 'center', padding: '56px 20px'}}>
            <div style={{width: '72px', height: '72px', background: 'transparent', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px'}}>
            ...
            {applications.map(app => {
              ...
              return (
                <Link key={app.id} to={`/applications/${app.id}`} style={{textDecoration: 'none', display: 'block'}}>
                  <div style={{padding: '20px', background: 'transparent', border: `1px solid ${styles.borderGlass}`, borderRadius: '18px', cursor: 'pointer', transition: 'border-color 0.2s'}} onMouseEnter={e => e.currentTarget.style.borderColor = styles.purpleBright} onMouseLeave={e => e.currentTarget.style.borderColor = styles.borderGlass}>
                    {/* Top row: name + badge */}
                    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', marginBottom: '16px'}}>
                      <div>
                      ...
                      <div style={{display: 'flex', alignItems: 'center', gap: '12px'}}>
                        ...
                        <span style={{padding: '4px 12px', fontSize: '10px', fontFamily: "'IBM Plex Mono', monospace", textTransform: 'uppercase', letterSpacing: '1px', borderRadius: '999px',
                    ...
                    <div style={{display: 'flex', gap: '3px', height: '4px', borderRadius: '2px', overflow: 'hidden'}}>
                      {STAGES.map((s, i) => (
                        <div key={s.key} style={{flex: 1, background: i <= idx ? stateColor(app.state) : 'rgba(255,255,255,0.05)', borderRadius: '2px'}} />
                      ))}
                    </div>
                  </div>
                </Link>
        ...
        <Panel>
          ...
          <div className="space-y-3">
            {certificates.map(cert => (
              <div key={cert.id} style={{padding: '16px', background: 'transparent', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px'}}>
                <div>
                ...
                <div style={{display: 'flex', gap: '8px'}}>
                  <a href={`${API_BASE}/api/certificates/${cert.certificate_number}/pdf`}
                     target="_blank"
                     style={{padding: '8px 16px', background: styles.purplePrimary, color: 'rgba(255,255,255,.94)', fontSize: '11px', fontFamily: "'IBM Plex Mono', monospace", textDecoration: 'none'}}>
                    Download PDF
                  </a>
                </div>
              </div>
    ...
    <div className="space-y-6" style={{maxWidth: "1200px", margin: "0 auto"}}>
      <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px', marginBottom: '28px'}}>
        ...
        <div style={{display: 'flex', gap: '10px'}}>
          <button onClick={() => loadData(true)} className="btn" style={{padding: '10px 16px'}}><RefreshCw fill="currentColor" fillOpacity={0.15} strokeWidth={1.8} size={14} style={refreshing ? {animation: "spin 0.8s linear infinite"} : {}} /> {refreshing ? "Refreshing..." : "Refresh"}</button>
        </div>
      </div>
      ...
      <Panel>
        ...
        <div style={{display: 'flex', gap: '4px', height: '32px', overflow: 'hidden'}}>
          ...
          {allApps.length === 0 && <div style={{flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.03)', padding: 'clamp(20px, 4vw, 40px) clamp(12px, 3vw, 20px)', textAlign: 'center'}}>
            <div style={{width: '48px', height: '48px', background: 'transparent', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px'}}><FileText fill="currentColor" fillOpacity={0.15} strokeWidth={1.8} size={22} style={{color: styles.purpleBright, opacity: 0.6}} /></div>
            ...
            {needsAction.slice(0, 5).map(app => (
              <div key={app.id} style={{padding: '14px 16px', background: 'transparent', border: `1px solid ${styles.borderGlass}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px'}}>
                <div style={{display: 'flex', alignItems: 'center', gap: '16px'}}>
                  ...
                  <span className="px-2 py-1" style={{background: 'transparent', color: styles.accentAmber, fontFamily: "'IBM Plex Mono', monospace", fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase'}}>{app.state?.replace('_', ' ')}</span>
                </div>
                <div style={{display: 'flex', gap: '8px'}}>
                  {app.state === 'pending' && (
                  ...
                  <button onClick={() => handleQuickAdvance(app.id, 'approved', `Approve ${app.system_name}`)} className="px-3 py-1" style={{background: 'transparent', border: '1px solid rgba(255,255,255,0.07)', color: styles.accentGreen, fontFamily: "'IBM Plex Mono', monospace", fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>Approve</button>
                  <button onClick={() => handleQuickAdvance(app.id, 'suspended', `Suspend ${app.system_name}`)} className="px-3 py-1" style={{background: 'transparent', border: '1px solid rgba(255,255,255,0.07)', color: styles.accentRed, fontFamily: "'IBM Plex Mono', monospace", fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>Suspend</button>
                  <Link to={`/applications/${app.id}`} className="px-3 py-1 no-underline" style={{background: 'transparent', border: `1px solid ${styles.borderGlass}`, color: styles.purpleBright, fontFamily: "'IBM Plex Mono', monospace", fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase'}}>View</Link>
                </div>
              </div>
            ))}
          </div>
        ...
        <Panel>
          ...
          <div className="space-y-3">
            {activeTests.map((test) => {
              ...
              return (
                <div key={test.id} className="p-4" style={{background: 'rgba(255,255,255,0.03)', border: `1px solid ${styles.borderGlass}`}}>
      ...
      {(() => {
        ...
        return (
          <div style={{background: 'transparent', border: '1px solid rgba(255,255,255,0.06)', padding: '16px'}}>
              ...
              <tbody>
                {recentCerts.slice(0, 5).map(c => (
                  <tr key={c.id} style={{borderBottom: `1px solid ${styles.borderGlass}`}}>
                    ...
                    <td className="px-4 py-3"><span style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '1px', textTransform: 'uppercase', padding: '2px 8px', background: c.state === 'conformant' ? 'rgba(92,214,133,0.04)' : 'rgba(214,92,92,0.04)', color: c.state === 'conformant' ? styles.accentGreen : '#D65C5C'}}>{c.state}</span></td>
                ...
                <tr key={app.id} className="transition-colors cursor-pointer" style={{borderBottom: `1px solid ${styles.borderGlass}`}} onClick={() => window.location.hash = `#/applications/${app.id}`}>
                  ...
                  <td className="px-4 py-3">
                    <span className="px-2 py-1 text-xs" style={{
                      background: app.state === 'conformant' ? 'rgba(92,214,133,0.04)' : app.state === 'observe' ? 'rgba(157,140,207,0.04)' : app.state === 'revoked' ? 'rgba(214,92,92,0.04)' : 'rgba(214,160,92,0.04)',
