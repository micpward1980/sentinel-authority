    ...
    <div className="space-y-6" style={{maxWidth: 'min(700px, 95vw)', margin: '0 auto'}}>
      ...
      <Panel>
        ...
        <div style={{display: 'flex', flexDirection: 'column', gap: '12px', maxWidth: 'min(400px, 90vw)'}}>
          <div>
            ...
            <input type="text" value={profileForm.full_name} onChange={e => setProfileForm({...profileForm, full_name: e.target.value})} className="w-full px-4 py-3 outline-none sexy-input" style={{background: 'rgba(255,255,255,0.03)', border: `1px solid ${styles.borderGlass}`, color: styles.textPrimary, fontSize: '13px'}} />
          </div>
          <div>
            <label style={{fontSize: '11px', color: styles.textTertiary, display: 'block', marginBottom: '4px'}}>Organization</label>
            <input type="text" value={profileForm.organization} onChange={e => setProfileForm({...profileForm, organization: e.target.value})} className="w-full px-4 py-3 outline-none sexy-input" style={{background: 'rgba(255,255,255,0.03)', border: `1px solid ${styles.borderGlass}`, color: styles.textPrimary, fontSize: '13px'}} />
          </div>
          ...
          <button onClick={handleSaveProfile} disabled={profileSaving} className="sexy-btn" style={{padding: '10px 24px', background: styles.purplePrimary, border: `1px solid ${styles.purpleBright}`, color: 'rgba(255,255,255,.94)', fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: profileSaving ? 'wait' : 'pointer', opacity: profileSaving ? 0.7 : 1, alignSelf: 'flex-start'}}>
      ...
      <Panel>
        ...
        <div style={{display: 'flex', flexDirection: 'column', gap: '12px', maxWidth: 'min(400px, 90vw)'}}>
          <div>
            ...
            <input type="password" value={pwForm.current} onChange={e => setPwForm({...pwForm, current: e.target.value})} className="w-full px-4 py-3 outline-none sexy-input" style={{background: 'rgba(255,255,255,0.03)', border: `1px solid ${styles.borderGlass}`, color: styles.textPrimary, fontSize: '13px'}} />
          </div>
          <div>
            <label style={{fontSize: '11px', color: styles.textTertiary, display: 'block', marginBottom: '4px'}}>New Password</label>
            <input type="password" value={pwForm.new_pw} onChange={e => setPwForm({...pwForm, new_pw: e.target.value})} className="w-full px-4 py-3 outline-none sexy-input" style={{background: 'rgba(255,255,255,0.03)', border: `1px solid ${styles.borderGlass}`, color: styles.textPrimary, fontSize: '13px'}} />
          </div>
          <div>
            <label style={{fontSize: '11px', color: styles.textTertiary, display: 'block', marginBottom: '4px'}}>Confirm New Password</label>
            <input type="password" value={pwForm.confirm} onChange={e => setPwForm({...pwForm, confirm: e.target.value})} className="w-full px-4 py-3 outline-none sexy-input" style={{background: 'rgba(255,255,255,0.03)', border: `1px solid ${styles.borderGlass}`, color: styles.textPrimary, fontSize: '13px'}} />
          </div>
          ...
          <button onClick={handleChangePassword} disabled={pwSaving} className="sexy-btn" style={{padding: '10px 24px', background: styles.purplePrimary, border: `1px solid ${styles.purpleBright}`, color: 'rgba(255,255,255,.94)', fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: pwSaving ? 'wait' : 'pointer', opacity: pwSaving ? 0.7 : 1, alignSelf: 'flex-start'}}>
        ...
        {twoFA.loading ? (
          ...
          <div>
            ...
            {twoFA.backupCodes && twoFA.backupCodes.length > 0 && (
              <div style={{marginBottom: '16px', padding: '16px', background: 'transparent', border: '1px solid rgba(255,255,255,0.07)' }}>
                ...
                <div style={{display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '4px'}}>
                  {twoFA.backupCodes.map((code, i) => (
                    <span key={i} style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '13px', color: styles.textPrimary, padding: '4px 8px', background: 'transparent', textAlign: 'center'}}>{code}</span>
                  ))}
                </div>
                <button onClick={() => {navigator.clipboard.writeText(twoFA.backupCodes.join(String.fromCharCode(10))); toast.show('Backup codes copied');}} style={{marginTop: '12px', padding: '6px 16px', background: 'transparent', border: '1px solid rgba(255,255,255,0.07)', color: styles.accentAmber, fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', cursor: 'pointer', letterSpacing: '1px', textTransform: 'uppercase'}}>Copy All</button>
              </div>
            )}
            <div style={{display: 'flex', gap: '8px', alignItems: 'center', maxWidth: 'min(400px, 90vw)'}}>
              <input type="password" value={twoFA.disablePw} onChange={e => setTwoFA(prev => ({...prev, disablePw: e.target.value}))} placeholder="Enter password to disable" className="sexy-input" style={{flex: 1, background: 'rgba(255,255,255,0.03)', border: `1px solid ${styles.borderGlass}`, padding: '8px 12px', color: styles.textPrimary, fontSize: '13px'}} />
              <button onClick={disable2FA} disabled={twoFA.disabling} style={{padding: '8px 16px', background: 'transparent', border: '1px solid rgba(255,255,255,0.07)', color: '#D65C5C', fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '1px', textTransform: 'uppercase', cursor: twoFA.disabling ? 'wait' : 'pointer', whiteSpace: 'nowrap'}}>
          ...
          <div style={{maxWidth: 'min(400px, 90vw)'}}>
            ...
            <div style={{background: 'rgba(255,255,255,0.03)', border: `1px solid ${styles.borderGlass}`, padding: '12px', marginBottom: '16px'}}>
          ...
          <div>
            ...
            <button onClick={setup2FA} className="sexy-btn" style={{padding: '10px 24px', background: styles.purplePrimary, border: `1px solid ${styles.purpleBright}`, color: 'rgba(255,255,255,.94)', fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
              Enable 2FA
            </button>
          </div>
        )}
      ...
      <Panel>
        ...
        <div style={{display: 'flex', flexDirection: 'column', gap: '4px'}}>
          {cats.map(cat => (
            <div key={cat.key} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', padding: '16px', background: prefs[cat.key] ? 'rgba(92,214,133,0.03)' : 'rgba(255,255,255,0.01)', border: `1px solid ${prefs[cat.key] ? 'rgba(92,214,133,0.1)' : styles.borderGlass}`, transition: 'all 0.2s'}}>
              <div style={{flex: 1}}>
              ...
              <button onClick={() => togglePref(cat.key)} style={{width: '48px', height: '26px', border: 'none', cursor: 'pointer', background: prefs[cat.key] ? styles.accentGreen : 'rgba(255,255,255,0.1)', position: 'relative', transition: 'background 0.2s', flexShrink: 0, marginLeft: '16px'}}>
        ...
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', marginTop: '20px', paddingTop: '16px', borderTop: `1px solid ${styles.borderGlass}`}}>
          ...
          <button onClick={savePrefs} disabled={saving} className="sexy-btn" style={{padding: '10px 24px', background: saved ? 'rgba(92,214,133,0.04)' : styles.purplePrimary, border: `1px solid ${saved ? 'rgba(92,214,133,0.4)' : styles.purpleBright}`, color: saved ? styles.accentGreen : '#fff', fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: saving ? 'wait' : 'pointer', opacity: saving ? 0.7 : 1}}>
