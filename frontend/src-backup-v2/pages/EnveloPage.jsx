    ...
    <div>
      <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px'}}>
        <div style={{padding: '16px', background: 'transparent', textAlign: 'center'}}>
          <div style={{fontSize: 'clamp(20px, 4vw, 28px)', fontWeight: 500, color: 'rgba(255,255,255,.94)'}}>{(session.pass_count || 0) + (session.block_count || 0)}</div>
        ...
        <div style={{padding: '16px', background: passRate >= 95 ? 'rgba(92,214,133,0.1)' : 'rgba(214,92,92,0.03)', textAlign: 'center'}}>
      ...
      <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px'}}>
        ...
        <div><div style={{fontSize: '11px', color: '#888', marginBottom: '4px'}}>STATUS</div><span style={{padding: '4px 12px', fontSize: '12px', background: session.status === 'active' ? 'rgba(92,214,133,0.2)' : 'rgba(255,255,255,0.1)', color: session.status === 'active' ? '#5CD685' : '#888'}}>{session.status?.toUpperCase()}</span></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{maxHeight: '300px', overflowY: 'auto'}}>
      <table style={{width: '100%', borderCollapse: 'collapse', fontSize: '12px'}}>
        ...
        <tbody>{records.map((r, i) => (<tr key={i} style={{borderBottom: '1px solid rgba(255,255,255,0.05)'}}><td style={{padding: '8px', fontFamily: 'monospace', fontSize: '11px', color: '#aaa'}}>{r.timestamp ? new Date(r.timestamp).toLocaleTimeString() : '-'}</td><td style={{padding: '8px', color: 'rgba(255,255,255,.94)'}}>{r.action_type}</td><td style={{padding: '8px'}}><span style={{padding: '2px 8px', fontSize: '10px', fontWeight: 400, background: r.result === 'PASS' ? 'rgba(92,214,133,0.2)' : 'rgba(214,92,92,0.2)', color: r.result === 'PASS' ? '#5CD685' : '#D65C5C'}}>{r.result}</span></td><td style={{padding: '8px', color: '#666', fontFamily: 'monospace', fontSize: '10px'}}>{JSON.stringify(r.parameters || {})}</td></tr>))}</tbody>
      </table>
    </div>
  );
}
    ...
    <div>
      {generatedKey && (
        <div style={{background: 'transparent', border: '1px solid rgba(255,255,255,0.07)', padding: '16px', marginBottom: '20px'}}>
          ...
          <div style={{background: 'rgba(255,255,255,.05)', padding: '12px', fontFamily: "'IBM Plex Mono', monospace", fontSize: '13px', color: styles.textPrimary, wordBreak: 'break-all', marginBottom: '12px'}}>
            {generatedKey.key}
          </div>
          <div style={{display: 'flex', gap: '12px'}}>
            <button onClick={copyKey} style={{padding: '8px 16px', background: styles.purplePrimary, border: `1px solid ${styles.purpleBright}`, color: 'rgba(255,255,255,.94)', fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', cursor: 'pointer'}}>Copy to Clipboard</button>
            <button onClick={() => setGeneratedKey(null)} style={{padding: '8px 16px', background: 'transparent', border: `1px solid ${styles.borderGlass}`, color: styles.textSecondary, fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', cursor: 'pointer'}}>Dismiss</button>
          </div>
          ...
          <div style={{marginTop: '16px', padding: '16px', background: 'rgba(91,75,138,0.2)', border: '1px solid rgba(91,75,138,0.3)' }}>
            ...
            <button 
              ...
              style={{padding: '12px 24px', background: styles.purplePrimary, border: `1px solid ${styles.purpleBright}`, color: 'rgba(255,255,255,.94)', fontFamily: "'IBM Plex Mono', monospace", fontSize: '12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px'}}
            >
      ...
      <div style={{display: 'flex', gap: '12px', marginBottom: '20px'}}>
        <input
          ...
          style={{flex: 1, padding: '10px 14px', background: 'rgba(255,255,255,.05)', border: `1px solid ${styles.borderGlass}`, color: styles.textPrimary, fontFamily: "'IBM Plex Mono', monospace", fontSize: '13px'}}
        />
        <button
          onClick={generateKey}
          disabled={!newKeyName.trim()}
          style={{padding: '10px 20px', background: newKeyName.trim() ? styles.purplePrimary : 'transparent', border: `1px solid ${newKeyName.trim() ? styles.purpleBright : styles.borderGlass}`, color: newKeyName.trim() ? '#fff' : styles.textTertiary, fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: newKeyName.trim() ? 'pointer' : 'not-allowed'}}
        >
          Generate Key
        </button>
      </div>
          ...
          {keys.map((k) => (
            <div key={k.id} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', padding: '12px 16px', background: 'transparent', border: `1px solid ${styles.borderGlass}`, marginBottom: '8px'}}>
              ...
              <div style={{display: 'flex', alignItems: 'center', gap: '16px'}}>
                ...
                <button onClick={() => revokeKey(k.id)} style={{padding: '6px 12px', background: 'rgba(255,100,100,0.1)', border: '1px solid rgba(255,100,100,0.3)', color: '#ff6464', fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', cursor: 'pointer'}}>Revoke</button>
              </div>
            </div>
          ))}
        </div>
...
function BoundaryConfigurator({ certificateNumber, initialBoundaries, onSave }) {
  ...
  const inputStyle = { background: 'rgba(255,255,255,0.03)', border: '1px solid ' + styles.borderGlass, color: styles.textPrimary, fontFamily: "'IBM Plex Mono', monospace", fontSize: '12px', outline: 'none' };
  const sectionTitle = (text) => <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.purpleBright, marginBottom: '8px', marginTop: '20px' }}>{text}</div>;
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', marginBottom: '20px', padding: '12px 16px', background: dirty ? 'rgba(214,160,92,0.03)' : 'rgba(92,214,133,0.05)', border: '1px solid ' + (dirty ? 'rgba(214,160,92,0.2)' : 'rgba(92,214,133,0.04)') }}>
        <div style={{ display: 'flex', gap: '24px', alignItems: 'center' }}>
          <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', color: styles.textTertiary }}>{totalCount} boundaries defined</span>
          {dirty && <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.accentAmber, letterSpacing: '1px', textTransform: 'uppercase' }}>● Unsaved changes</span>}
        </div>
        <button onClick={handleSave} disabled={saving || !dirty} style={{ padding: '8px 24px', background: dirty ? styles.purplePrimary : 'rgba(255,255,255,0.05)', border: '1px solid ' + (dirty ? styles.purpleBright : styles.borderGlass), color: dirty ? '#fff' : styles.textTertiary, fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: dirty ? 'pointer' : 'default', opacity: saving ? 0.6 : 1 }}>
                ...
                {pendingApps.map(app => (
                  <div key={app.id} style={{padding: '16px', background: 'transparent', border: `1px solid ${styles.borderGlass}`}}>
                    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px'}}>
                      ...
                      <span style={{padding: '4px 12px', background: 'transparent', border: '1px solid rgba(255,255,255,0.07)', fontSize: '10px', color: styles.accentAmber, fontFamily: "'IBM Plex Mono', monospace", textTransform: 'uppercase'}}>
          ...
          {reviewingCert ? (() => {
            ...
            return (
              <Panel glow>
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px'}}>
                  <div>
                    <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.purpleBright, marginBottom: '8px'}}>Boundary Review — Read Only</p>
                ...
                {!hasBoundaries ? (
                  ...
                  <div className="space-y-6">
                    {numeric.length > 0 && (
                      <div>
                        <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '12px'}}>Numeric Boundaries ({numeric.length})</p>
                        <div style={{display: 'grid', gap: '8px'}}>
                          {numeric.map((b, i) => (
                            <div key={i} style={{padding: '14px 16px', background: 'transparent', border: `1px solid ${styles.borderGlass}`, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '16px', alignItems: 'center'}}>
                              <div>
                                <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary, marginBottom: '2px'}}>Parameter</p>
                                <p style={{fontWeight: 500, color: styles.textPrimary, fontSize: '14px'}}>{b.name || b.parameter || '—'}</p>
                              </div>
                              <div>
                                <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary, marginBottom: '2px'}}>Range</p>
                                <p style={{color: styles.purpleBright, fontFamily: "'IBM Plex Mono', monospace", fontSize: '13px'}}>{b.min_value ?? '—'} → {b.max_value ?? '—'} {b.unit || ''}</p>
                              </div>
                              <div>
                                <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary, marginBottom: '2px'}}>Hard Limit</p>
                                <p style={{color: b.hard_limit ? styles.accentRed : styles.textTertiary, fontFamily: "'IBM Plex Mono', monospace", fontSize: '13px'}}>{b.hard_limit ?? 'None'}</p>
                              </div>
                              <div>
                                <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary, marginBottom: '2px'}}>Tolerance</p>
                                <p style={{color: styles.textSecondary, fontFamily: "'IBM Plex Mono', monospace", fontSize: '13px'}}>±{b.tolerance || 0}</p>
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    {geo.length > 0 && (
                      <div>
                        <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '12px'}}>Geographic Boundaries ({geo.length})</p>
                        <div style={{display: 'grid', gap: '8px'}}>
                          {geo.map((b, i) => (
                            <div key={i} style={{padding: '14px 16px', background: 'transparent', border: `1px solid ${styles.borderGlass}`, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '16px', alignItems: 'center'}}>
                              <div>
                                <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary, marginBottom: '2px'}}>Zone</p>
                                <p style={{fontWeight: 500, color: styles.textPrimary, fontSize: '14px'}}>{b.name || '—'}</p>
                              </div>
                              <div>
                                <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary, marginBottom: '2px'}}>Type</p>
                                <p style={{color: styles.textSecondary, fontSize: '13px'}}>{b.boundary_type || 'circle'}</p>
                              </div>
                              <div>
                                <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary, marginBottom: '2px'}}>Center</p>
                                <p style={{color: styles.purpleBright, fontFamily: "'IBM Plex Mono', monospace", fontSize: '12px'}}>{(b.center?.lat || b.lat || '—')}, {(b.center?.lon || b.lon || '—')}</p>
                              </div>
                              <div>
                                <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary, marginBottom: '2px'}}>Radius</p>
                                <p style={{color: styles.textSecondary, fontFamily: "'IBM Plex Mono', monospace", fontSize: '13px'}}>{b.radius_meters || '—'}m</p>
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    {time.length > 0 && (
                      <div>
                        <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '12px'}}>Time Boundaries ({time.length})</p>
                        <div style={{display: 'grid', gap: '8px'}}>
                          {time.map((b, i) => (
                            <div key={i} style={{padding: '14px 16px', background: 'transparent', border: `1px solid ${styles.borderGlass}`, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '16px', alignItems: 'center'}}>
                              <div>
                                <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary, marginBottom: '2px'}}>Schedule</p>
                                <p style={{fontWeight: 500, color: styles.textPrimary, fontSize: '14px'}}>{b.name || '—'}</p>
                              </div>
                              <div>
                                <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary, marginBottom: '2px'}}>Hours</p>
                                <p style={{color: styles.purpleBright, fontFamily: "'IBM Plex Mono', monospace", fontSize: '13px'}}>{b.allowed_hours_start ?? b.start_hour ?? 0}:00 → {b.allowed_hours_end ?? b.end_hour ?? 24}:00 {b.timezone || 'UTC'}</p>
                              </div>
                              <div>
                                <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary, marginBottom: '2px'}}>Days</p>
                    ...
                    {state.length > 0 && (
                      <div>
                        <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '12px'}}>State Boundaries ({state.length})</p>
                        <div style={{display: 'grid', gap: '8px'}}>
                          {state.map((b, i) => (
                            <div key={i} style={{padding: '14px 16px', background: 'transparent', border: `1px solid ${styles.borderGlass}`, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '16px', alignItems: 'center'}}>
                              <div>
                              ...
                              <div>
                                <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary, marginBottom: '2px'}}>Allowed</p>
                                <p style={{color: styles.accentGreen, fontSize: '12px'}}>{(b.allowed_values || []).join(', ') || '—'}</p>
                              </div>
                              <div>
                                <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary, marginBottom: '2px'}}>Forbidden</p>
                ...
                <div style={{marginTop: '32px', paddingTop: '24px', borderTop: `1px solid ${styles.borderGlass}`}}>
                  <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '16px'}}>Review Decision</p>
                  ...
                  <textarea
                    ...
                    style={{
                      ...
                      fontFamily: "'Source Serif 4', serif", resize: 'vertical'
                    }}
                  />
                  ...
                  <div style={{display: 'flex', gap: '12px', marginTop: '16px'}}>
                    <button
                      ...
                      style={{
                        ...
                        border: `1px solid ${styles.accentGreen}`, color: styles.accentGreen, fontFamily: "'IBM Plex Mono', monospace",
                    ...
                    <button
                      ...
                      style={{
                        flex: 1, padding: '14px', background: 'transparent',
                        border: '1px solid rgba(255,255,255,0.07)', color: styles.accentRed, fontFamily: "'IBM Plex Mono', monospace",
                        fontSize: '12px', letterSpacing: '1px', textTransform: 'uppercase',
                        cursor: 'pointer'
                      }}
                    >
            ...
            <Panel>
              <div style={{textAlign: 'center', padding: 'clamp(24px, 5vw, 60px) clamp(12px, 3vw, 20px)'}}>
                ...
                <button onClick={() => setActiveTab('customers')} style={{padding: '12px 24px', background: styles.purplePrimary, border: `1px solid ${styles.purpleBright}`, color: 'rgba(255,255,255,.94)', fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>View Customer Systems</button>
              </div>
            </Panel>
          )}
        </>
    ...
    return (
      <div className="space-y-6">
        ...
        <Panel>
          <div style={{textAlign: 'center', padding: 'clamp(24px, 5vw, 60px) clamp(12px, 3vw, 20px)'}}>
            <Shield fill="currentColor" fillOpacity={0.15} strokeWidth={1.8} size={48} style={{color: styles.textTertiary, margin: '0 auto 16px'}} />
            <h2 style={{fontFamily: "'Source Serif 4', serif", fontSize: 'clamp(18px, 4vw, 24px)', fontWeight: 200, marginBottom: '12px'}}>Pending Approval</h2>
  ...
  if (isDeployed) {
    return (
      <div className="space-y-6">
            ...
            <Panel key={cert.id} glow={isOnline}>
              <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '20px'}}>
                ...
                <div style={{display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 16px', background: 'transparent' }}>
                  ...
                  <span style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', textTransform: 'uppercase', color: styles.accentGreen}}>ENVELO Active</span>
                </div>
              </div>
              {session && (
            ...
            <Panel key={app.id} glow>
              <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px'}}>
                <div>
                  ...
                  <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '13px', color: styles.accentAmber}}>{app.application_number} — CAT-72 Testing</p>
                </div>
                <div style={{display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 16px', background: 'transparent' }}>
                  <div style={{width: '8px', height: '8px', borderRadius: '50%', background: styles.accentGreen, animation: 'pulse 2s infinite'}}></div>
                  <span style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', textTransform: 'uppercase', color: styles.accentGreen}}>ENVELO Active</span>
              ...
              {session && (
                <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', marginTop: '24px', paddingTop: '24px', borderTop: '1px solid ' + styles.borderGlass}}>
                  <div style={{textAlign: 'center'}}>
                    ...
                    <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary}}>Passed</p>
                  </div>
                  <div style={{textAlign: 'center'}}>
                    ...
                    <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary}}>Blocked</p>
                  </div>
                  <div style={{textAlign: 'center'}}>
                    ...
                    <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary}}>Total</p>
                  </div>
                </div>
              )}
            </Panel>
        ...
        <Panel>
          <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '16px'}}>Agent Control</p>
          ...
          {showUninstall && (
            <div style={{marginTop: '16px', padding: '16px', background: 'rgba(255,255,255,.05)', border: '1px solid ' + styles.borderGlass}}>
              <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '1px', textTransform: 'uppercase', color: styles.accentAmber, marginBottom: '8px'}}>Paste in terminal to fully remove</p>
              <div style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '12px', color: styles.textSecondary, padding: '12px', background: 'rgba(255,255,255,.05)', whiteSpace: 'pre-wrap', lineHeight: '1.8'}}>
          ...
          <div style={{marginTop: '16px', padding: '12px', background: 'transparent' }}>
            <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '1px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '8px'}}>Logs</p>
            <div style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '12px', color: styles.textSecondary, padding: '8px', background: 'rgba(255,255,255,.05)' }}>cat ~/.envelo/envelo.log</div>
          </div>
        </Panel>
      </div>
    );
        ...
        <div style={{textAlign: 'center', padding: 'clamp(20px, 4vw, 40px) clamp(12px, 3vw, 20px)'}}>
          {!deployKey ? (
            <>
              ...
              <h2 style={{fontFamily: "'Source Serif 4', serif", fontSize: 'clamp(20px, 4vw, 28px)', fontWeight: 200, marginBottom: '12px', color: styles.textPrimary}}>One Command. That's It.</h2>
            ...
            <>
              <div style={{marginBottom: '24px'}}>
                <div style={{display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '8px 16px', background: 'transparent', marginBottom: '16px'}}>
                  ...
                  <span style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', color: styles.accentGreen, textTransform: 'uppercase', letterSpacing: '1px'}}>Ready to Deploy</span>
                </div>
                <h2 style={{fontFamily: "'Source Serif 4', serif", fontSize: 'clamp(18px, 4vw, 24px)', fontWeight: 200, color: styles.textPrimary, margin: '0 0 8px 0'}}>Paste in your terminal</h2>
              </div>
              ...
              <div style={{maxWidth: 'min(700px, 95vw)', margin: '0 auto', textAlign: 'left'}}>
                <div style={{background: 'rgba(255,255,255,.05)', border: '1px solid ' + styles.purpleBright, overflow: 'hidden'}}>
                  ...
                  <div style={{padding: '20px', fontFamily: "'IBM Plex Mono', monospace", fontSize: '13px', lineHeight: '1.6', overflowX: 'auto', whiteSpace: 'pre-wrap', wordBreak: 'break-all'}}>
                ...
                <div style={{marginTop: '20px', padding: '16px', background: 'transparent', border: '1px solid ' + styles.borderGlass}}>
                  <p style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '1px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '12px'}}>What happens</p>
                ...
                <div style={{textAlign: 'center', marginTop: '16px'}}>
                  <button onClick={() => setDeployKey(null)} style={{background: 'transparent', border: 'none', color: styles.textTertiary, fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', cursor: 'pointer', textDecoration: 'underline'}}>
                    Generate new key
                  </button>
                </div>
              </div>
