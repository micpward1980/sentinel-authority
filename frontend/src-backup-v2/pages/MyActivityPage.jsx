    ...
    <div className="space-y-6" style={{maxWidth: 'min(800px, 95vw)', margin: '0 auto'}}>
      ...
      <Panel>
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px', marginBottom: '16px'}}>
          <span style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', color: styles.textTertiary, letterSpacing: '1px', textTransform: 'uppercase'}}>Activity Log</span>
          <div style={{display: 'flex', gap: '6px', alignItems: 'center'}}>
            {['7', '30', '90', 'all'].map(r => (
              <button key={r} onClick={() => { setDateRange(r); setPage(0); }} style={{padding: '4px 10px', background: dateRange === r ? 'rgba(168,150,214,0.15)' : 'transparent', border: `1px solid ${dateRange === r ? 'rgba(168,150,214,0.4)' : styles.borderGlass}`, color: dateRange === r ? styles.purpleBright : styles.textTertiary, fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', cursor: 'pointer', letterSpacing: '0.5px', transition: 'all 0.15s ease'}}>
                {r === 'all' ? 'All' : r + 'd'}
              </button>
            ))}
            <span style={{fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary, marginLeft: '8px'}}>{total} entries</span>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div style={{display: 'flex', flexDirection: 'column', gap: '2px'}}>
            {logs.map(log => {
              ...
              return (
                <div key={log.id} style={{display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '12px', background: 'rgba(255,255,255,0.01)', borderBottom: `1px solid ${styles.borderSubtle}`}}>
