  ...
  return (
    <div style={{minHeight: '100vh', color: styles.textPrimary, fontFamily: styles.sans}}>
      ...
      {isMobile && sidebarOpen && (
        <div onClick={() => setSidebarOpen(false)} style={{position: 'fixed', inset: 0, background: 'rgba(255,255,255,.05)', zIndex: 40}} />
      )}
      ...
      <div
        ...
        <div style={{position: 'absolute', bottom: 0, left: 0, right: 0, padding: '16px', borderTop: '1px solid rgba(255,255,255,0.06)'}}>
          <div style={{display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px'}}>
            <div style={{width: '28px', height: '28px', background: styles.purplePrimary, border: '1px solid rgba(157,140,207,0.3)', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
              <span style={{color: 'rgba(255,255,255,.94)', fontSize: '11px', fontWeight: 500}}>{user?.full_name?.[0] || 'U'}</span>
            </div>
            <div>
        ...
        <header style={{
          ...
          <div style={{position: 'relative'}}>
            <button onClick={() => setNotifOpen(!notifOpen)} style={{background: 'none', border: 'none', cursor: 'pointer', padding: '4px', color: notifOpen ? styles.purpleBright : styles.textTertiary}}>
              ...
              {unreadCount > 0 && <span data-dot="true" style={{position: 'absolute', top: '-2px', right: '-2px', minWidth: '14px', height: '14px', borderRadius: '50%', background: '#D65C5C', color: 'rgba(255,255,255,.94)', fontSize: '8px', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: styles.mono, padding: '0 2px'}}>{unreadCount > 9 ? '9+' : unreadCount}</span>}
            </button>
            {notifOpen && (<>
              <div onClick={() => setNotifOpen(false)} style={{position: 'fixed', inset: 0, zIndex: 90}} />
              <div style={{position: 'absolute', right: 0, top: '44px', width: 'min(340px, 90vw)', maxHeight: '70vh', overflowY: 'auto', background: 'rgba(42,47,61,0.96)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '18px', zIndex: 100}}>
