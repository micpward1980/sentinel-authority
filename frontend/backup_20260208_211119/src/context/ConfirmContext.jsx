    ...
    <ConfirmContext.Provider value={confirm}>
      ...
      {state.open && (
        <div style={{position: 'fixed', inset: 0, zIndex: 99999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)'}}>
          <div style={{background: styles.bgCard, border: `1px solid ${styles.borderGlass}`, maxWidth: '420px', width: '90%', overflow: 'hidden'}}>
            <div style={{padding: '24px 24px 16px'}}>
              <h3 style={{margin: '0 0 8px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', color: state.danger ? '#D65C5C' : styles.purpleBright}}>{state.title}</h3>
            ...
            <div style={{padding: '16px 24px', display: 'flex', justifyContent: 'flex-end', gap: '10px', borderTop: `1px solid ${styles.borderGlass}`}}>
              <button onClick={state.onCancel} style={{padding: '8px 20px', background: 'transparent', border: `1px solid ${styles.borderGlass}`, color: styles.textSecondary, fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>Cancel</button>
              <button onClick={state.onConfirm} style={{padding: '8px 20px', background: state.danger ? 'rgba(214,92,92,0.2)' : styles.purplePrimary, border: `1px solid ${state.danger ? 'rgba(214,92,92,0.4)' : styles.purpleBright}`, color: state.danger ? '#D65C5C' : '#fff', fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>{state.confirmLabel}</button>
            </div>
          </div>
        </div>
      )}
