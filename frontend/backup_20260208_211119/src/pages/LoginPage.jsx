  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden" style={{background: styles.bgDeep}}>
      <Link to="/dashboard" style={{
        ...
        transition: 'color 0.2s'}}>← Dashboard</Link>
      ...
      <style>{`
        ...
        .login-input:focus { border-color: rgba(157,140,207,0.6) !important; box-shadow: 0 0 0 3px rgba(157,140,207,0.1), 0 4px 20px transparent; transform: translateY(-1px); }
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-10">
          ...
          <div className="relative flex justify-center mb-6" style={{height: '100px', alignItems: 'center'}}>
            ...
            <div style={{
              ...
              <div style={{
                ...
                borderRadius: '50%', animation: 'eyePulse 3s ease-in-out infinite'}} />
            </div>
          </div>
          ...
          <p style={{
            color: styles.textTertiary, fontFamily: "Consolas, 'IBM Plex Mono', monospace",
            fontSize: '10px', letterSpacing: '4px', textTransform: 'uppercase', marginTop: '12px'}}>Sentinel Authority</p>
          ...
          <a href="https://sentinelauthority.org" style={{
            ...
            marginTop: '16px', display: 'inline-block'}}>← VISIT MAIN SITE</a>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <button onClick={() => setIsRegister(false)} type="button" style={{
              ...
              letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>Sign In</button>
            <button onClick={() => setIsRegister(true)} type="button" style={{
              flex: 1, padding: '12px', border: 'none',
              background: isRegister ? styles.purplePrimary : 'transparent',
              color: isRegister ? '#fff' : styles.textTertiary,
              fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '11px',
              letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>Register</button>
          </div>

                  {twoFAStep ? (
          <div style={{textAlign: 'center'}}>
            <div style={{width: '48px', height: '48px', background: 'rgba(157,140,207,0.05)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px'}}>
            ...
            {error && <div style={{background: 'transparent', border: '1px solid rgba(214,92,92,0.2)', padding: '10px 14px', marginBottom: '16px', color: '#D65C5C', fontSize: '13px'}}>{error}</div>}
            <form onSubmit={handle2FASubmit}>
              <input type="text" value={totpCode} onChange={e => setTotpCode(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="000000" maxLength={6} autoFocus style={{width: '100%', background: 'transparent', border: '1px solid ' + styles.borderGlass, padding: '14px', color: styles.textPrimary, fontSize: '24px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", letterSpacing: '8px', textAlign: 'center', outline: 'none', marginBottom: '16px', boxSizing: 'border-box'}} />
              <button type="submit" disabled={verifying2FA || totpCode.length !== 6} className="login-btn" style={{width: '100%', padding: '14px', background: totpCode.length === 6 ? styles.purplePrimary : 'rgba(255,255,255,0.05)', border: '1px solid ' + (totpCode.length === 6 ? styles.purpleBright : styles.borderGlass), color: totpCode.length === 6 ? '#fff' : styles.textTertiary, fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', cursor: verifying2FA ? 'wait' : 'pointer'}}>
...
{error && (
            <div style={{
              marginBottom: '24px', padding: '14px 16px', background: 'transparent', border: 'none',
              color: styles.accentRed, fontSize: '13px', display: 'flex', alignItems: 'center', gap: '10px'}}><span style={{fontSize: '16px'}}>⚠</span>{error}</div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <>
                <div>
                  <label style={{
                    ...
                    letterSpacing: '1px', textTransform: 'uppercase'}}>Full Name</label>
                  <input type="text" placeholder="Jane Smith" value={formData.full_name}
                    onChange={(e) => setFormData({...formData, full_name: e.target.value})}
                    className="login-input w-full px-4 py-4 outline-none"
                    style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.08)',
                      color: styles.textPrimary, fontFamily: "'Inter', sans-serif", fontSize: '15px'}} required />
                </div>
                <div>
                  <label style={{
                    display: 'block', marginBottom: '8px', color: styles.textTertiary,
                    fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '10px',
                    letterSpacing: '1px', textTransform: 'uppercase'}}>Organization</label>
            ...
            <div>
              <label style={{
                display: 'block', marginBottom: '8px', color: styles.textTertiary,
                fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '10px',
                letterSpacing: '1px', textTransform: 'uppercase'}}>Email Address</label>
            ...
            <div>
              <label style={{
                display: 'block', marginBottom: '8px', color: styles.textTertiary,
                fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '10px',
                letterSpacing: '1px', textTransform: 'uppercase'}}>Password</label>
            ...
            <button type="submit" className="login-btn w-full py-4 font-medium"
              style={{
                background: 'transparent',
                border: '1px solid rgba(157,140,207,0.5)', color: '#fff',
                fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '12px',
                letterSpacing: '2px', textTransform: 'uppercase', cursor: 'pointer',
                marginTop: '8px'}}>{isRegister ? 'Create Account' : 'Sign In'}</button>
          </form>
        </>)}
