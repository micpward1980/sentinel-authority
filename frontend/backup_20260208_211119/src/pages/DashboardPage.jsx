      ...
      <Panel>
        ...
        {applications.length === 0 ? (
          <div style={{textAlign: 'center', padding: '56px 20px'}}>
            <div style={{width: '72px', height: '72px', background: 'rgba(91,75,138,0.06)', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px'}}>
            ...
            <Link to="/applications/new" className="inline-flex items-center gap-2 px-6 py-3 no-underline" style={{background: styles.purplePrimary, border: 'none', color: '#fff', fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase' }}>
                  ...
                  <div style={{padding: '20px', background: 'transparent', border: `1px solid ${styles.borderGlass}`, cursor: 'pointer', transition: 'border-color 0.2s'}} onMouseEnter={e => e.currentTarget.style.borderColor = styles.purpleBright} onMouseLeave={e => e.currentTarget.style.borderColor = styles.borderGlass}>
                    ...
                    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', marginBottom: '16px'}}>
                      ...
                      <div style={{display: 'flex', alignItems: 'center', gap: '12px'}}>
                        ...
                        <span style={{padding: '4px 12px', fontSize: '10px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", textTransform: 'uppercase', letterSpacing: '1px',
                    ...
                    <div style={{display: 'flex', gap: '3px', height: '4px'}}>
                      {STAGES.map((s, i) => (
                        <div key={s.key} style={{flex: 1, background: i <= idx ? stateColor(app.state) : 'rgba(255,255,255,0.05)'}} />
                      ))}
                    </div>
                  </div>
                </Link>
              ...
              <div key={cert.id} style={{padding: '16px', background: 'transparent', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px'}}>
                ...
                <div style={{display: 'flex', gap: '8px'}}>
                  <a href={`${API_BASE}/api/certificates/${cert.certificate_number}/pdf`}
                     target="_blank"
                     style={{padding: '8px 16px', background: styles.purplePrimary, color: '#fff', fontSize: '11px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", textDecoration: 'none'}}>
                    Download PDF
                  </a>
                </div>
              </div>
      ...
      <Panel>
        ...
        <div style={{display: 'flex', gap: '4px', height: '32px', overflow: 'hidden'}}>
          ...
          {allApps.length === 0 && <div style={{flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.03)', padding: 'clamp(20px, 4vw, 40px) clamp(12px, 3vw, 20px)', textAlign: 'center'}}>
            <div style={{width: '48px', height: '48px', background: 'rgba(91,75,138,0.06)', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px'}}><FileText fill="currentColor" fillOpacity={0.15} strokeWidth={1.8} size={22} style={{color: styles.purpleBright, opacity: 0.6}} /></div>
            ...
            {needsAction.slice(0, 5).map(app => (
              <div key={app.id} style={{padding: '14px 16px', background: 'transparent', border: `1px solid ${styles.borderGlass}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px'}}>
                <div style={{display: 'flex', alignItems: 'center', gap: '16px'}}>
                  ...
                  <span className="px-2 py-1" style={{background: 'rgba(214,160,92,0.05)', color: styles.accentAmber, fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase'}}>{app.state?.replace('_', ' ')}</span>
                </div>
                <div style={{display: 'flex', gap: '8px'}}>
                  {app.state === 'pending' && (
                    <button onClick={() => handleQuickAdvance(app.id, 'under_review', `Begin review for ${app.system_name}`)} className="px-3 py-1" style={{background: 'rgba(214,160,92,0.05)', border: '1px solid rgba(255,255,255,0.07)', color: styles.accentAmber, fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>Begin Review</button>
                  )}
                  <button onClick={() => handleQuickAdvance(app.id, 'approved', `Approve ${app.system_name}`)} className="px-3 py-1" style={{background: 'rgba(92,214,133,0.05)', border: '1px solid rgba(255,255,255,0.07)', color: styles.accentGreen, fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>Approve</button>
                  <button onClick={() => handleQuickAdvance(app.id, 'suspended', `Suspend ${app.system_name}`)} className="px-3 py-1" style={{background: 'transparent', border: '1px solid rgba(255,255,255,0.07)', color: styles.accentRed, fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>Suspend</button>
                  <Link to={`/applications/${app.id}`} className="px-3 py-1 no-underline" style={{background: 'rgba(157,140,207,0.04)', border: `1px solid ${styles.borderGlass}`, color: styles.purpleBright, fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase'}}>View</Link>
                </div>
              </div>
            ))}
          </div>
              ...
              <tbody>
                {recentCerts.slice(0, 5).map(c => (
                  <tr key={c.id} style={{borderBottom: `1px solid ${styles.borderGlass}`}}>
                    ...
                    <td className="px-4 py-3"><span style={{fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '1px', textTransform: 'uppercase', padding: '2px 8px', background: c.state === 'conformant' ? 'rgba(92,214,133,0.04)' : 'rgba(214,92,92,0.04)', color: c.state === 'conformant' ? styles.accentGreen : '#D65C5C'}}>{c.state}</span></td>
