                ...
                <div className="flex items-center gap-2">
                  ...
                  {user.role === 'pending' && (
                    <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                      ...
                      <button onClick={() => handleRejectUser(user.id, user.email)} className="px-2 py-1 text-xs" style={{background: 'rgba(214,92,92,0.05)', color: '#d65c5c', border: '1px solid rgba(255,255,255,0.07)', cursor: 'pointer'}}>Reject</button>
                    </div>
                  )}
                  {user.is_active === false && <span className="px-2 py-1 text-xs" style={{background: 'rgba(214,92,92,0.05)', color: styles.accentRed}}>Inactive</span>}
                </div>
              </div>
            ))}
          </div>
