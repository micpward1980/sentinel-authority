# Page snapshot

```yaml
- generic [ref=e3]:
  - generic [ref=e4]:
    - generic [ref=e5]:
      - img [ref=e6]
      - generic [ref=e9]: Sentinel Authority
    - heading "Sign In" [level=1] [ref=e10]:
      - text: Sign
      - emphasis [ref=e11]: In
    - paragraph [ref=e12]: Access the ODDC certification portal. Manage applications, certificates, and ENVELO Interlock.
    - generic [ref=e13]:
      - generic [ref=e14]:
        - button "Sign In" [active] [ref=e15] [cursor=pointer]
        - button "Register" [ref=e16] [cursor=pointer]
      - generic [ref=e17]:
        - generic [ref=e18]:
          - generic [ref=e19]: Email Address
          - textbox "you@company.com" [ref=e20]: info@sentinelauthority.org
        - generic [ref=e21]:
          - generic [ref=e22]: Password
          - generic [ref=e23]:
            - textbox "••••••••••••" [ref=e24]: Swsales1980!
            - button [ref=e25] [cursor=pointer]:
              - img [ref=e26]
        - button "Sign In →" [ref=e29] [cursor=pointer]
      - button "Forgot password?" [ref=e30] [cursor=pointer]
  - separator [ref=e31]
  - generic [ref=e32]:
    - link "← Public Site" [ref=e33] [cursor=pointer]:
      - /url: https://sentinelauthority.org
    - generic [ref=e34]:
      - link "Privacy" [ref=e35] [cursor=pointer]:
        - /url: https://sentinelauthority.org/privacy.html
      - link "Terms" [ref=e36] [cursor=pointer]:
        - /url: https://sentinelauthority.org/terms.html
```