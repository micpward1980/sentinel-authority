...
import { Bell, Settings, FileText, Activity, Award, Users, Home, LogOut, Menu, X, Search, ExternalLink, BookOpen, Clock, BarChart2 } from 'lucide-react';
...
function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen" style={{background: '#2a2f3d', color: styles.textPrimary, fontFamily: "Calibri, 'Inter', system-ui, -apple-system, sans-serif"}}>
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-64 transform transition-transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`} style={{background: 'rgba(42,47,61,0.88)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', borderRight: '1px solid rgba(255,255,255,0.08)', boxShadow: '4px 0 24px rgba(0,0,0,0.2)'}}>
        <div className="flex items-center justify-between h-16 px-4" style={{borderBottom: `1px solid ${styles.borderGlass}`}}>
          <Link to="/dashboard" className="flex items-center gap-3 no-underline">
            <BrandMark size={24} />
            <span style={{fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '10px', fontWeight: 500, letterSpacing: '3px', textTransform: 'uppercase', color: styles.textPrimary}}>Sentinel Authority</span>
          </Link>
        ...
        <nav className="p-4 space-y-1">
          {filteredNav.map((item) => (
            ...
            >
              {item.icon === 'brand' ? <BrandMark size={18} /> : <item.icon className="w-4 h-4" />}
              {item.name}
            </Link>
          ))}
        </nav>
        <div className="absolute bottom-0 left-0 right-0 p-4" style={{borderTop: `1px solid ${styles.borderGlass}`}}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <div style={{fontSize: '11px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", color: styles.textTertiary, textTransform: 'uppercase', letterSpacing: '1px'}}>{user?.role}</div>
            </div>
          </div>
          <button onClick={logout} className="flex items-center gap-2 w-full transition-colors" style={{color: styles.textTertiary, fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', background: 'none', border: 'none', cursor: 'pointer'}}>
      ...
      <div className="lg:ml-64 relative z-10">
        <header className="h-16 flex items-center px-6 gap-4" style={{borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(42,47,61,0.88)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)'}}>
          ...
          <a href="https://sentinelauthority.org" target="_blank" rel="noopener noreferrer" className="sa-header-links flex items-center gap-2 transition-colors no-underline" style={{color: styles.textTertiary, fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '1.5px', textTransform: 'uppercase'}}>
          ...
          <div style={{position: 'relative'}}>
            <button onClick={() => setNotifOpen(!notifOpen)} style={{background: 'none', border: 'none', cursor: 'pointer', position: 'relative', padding: '4px', color: notifOpen ? styles.purpleBright : styles.textTertiary, transition: 'color 0.2s'}}>
              <Bell fill="currentColor" fillOpacity={0.15} strokeWidth={1.8} className="w-5 h-5" />
              {unreadCount > 0 && <span style={{position: 'absolute', top: '-2px', right: '-2px', minWidth: '16px', height: '16px', borderRadius: '50%', background: '#D65C5C', color: '#fff', fontSize: '9px', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "Consolas, 'IBM Plex Mono', monospace", padding: '0 3px'}}>{unreadCount > 9 ? '9+' : unreadCount}</span>}
            </button>
            {notifOpen && (<>
                      ...
                      <div style={{display: 'flex', gap: '10px', alignItems: 'flex-start'}}>
                        ...
                        <div style={{flex: 1, minWidth: 0}}>
                          ...
                          <div style={{display: 'flex', gap: '8px', marginTop: '4px', flexWrap: 'wrap'}}>
                            <span style={{fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary}}>{n.timestamp ? new Date(n.timestamp).toLocaleString() : ''}</span>
                            {n.user_email && <span style={{fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '10px', color: styles.textTertiary}}>by {n.user_email}</span>}
                          </div>
                        </div>
