...
export default function EmptyState({ icon: Icon, title, message, description, action }) {
  ...
  return (
    <div style={{ textAlign: 'center', padding: '60px 20px', color: styles.textTertiary, fontFamily: styles.mono }}>
      {Icon && <div style={{ fontSize: '32px', marginBottom: '16px', opacity: 0.5 }}>
        {typeof Icon === 'string' ? Icon : <Icon size={32} strokeWidth={1.5} />}
      </div>}
      {title && <div style={{ fontFamily: styles.serif, fontSize: '22px', fontWeight: 200, color: styles.textSecondary, marginBottom: '8px' }}>{title}</div>}
