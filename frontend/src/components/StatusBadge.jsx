...
const ALIAS = { active:'green', conformant:'green', approved:'green', passed:'green', pending:'amber', bounded:'amber', review:'amber', warning:'amber', under_review:'amber', testing:'amber', revoked:'red', suspended:'red', failed:'red', error:'red' };
...
export default function StatusBadge({ status = '', label, children, showDot = true }) {
  const k = status.toLowerCase().replace(/[^a-z_]/g, '');
  const v = COLORS[k] || COLORS[ALIAS[k]] || DEF;
  return (
    <span className="badge" style={{ borderColor: v.bc, color: v.c, background: v.bg }}>
      {showDot && <span className="s-dot" style={{ background: v.dot }} />}
