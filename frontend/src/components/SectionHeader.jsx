    ...
    <div style={{marginBottom: '24px'}}>
      <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px'}}>
        <div>
          ...
          <h1 style={{fontFamily: styles.serif, fontSize: 'clamp(22px, 5vw, 32px)', fontWeight: 300, margin: 0, letterSpacing: '-0.04em', color: styles.purpleAccent}}>{title}</h1>
          {description && <p style={{color: styles.textSecondary, marginTop: '8px', fontSize: '16px', lineHeight: 1.85, fontWeight: 300, maxWidth: '580px'}}>{description}</p>}
        </div>
        {action}
      </div>
    </div>
