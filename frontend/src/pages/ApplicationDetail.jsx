    ...
    <div className="space-y-6">
      {emailPreview && (
        <div style={{position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)'}}>
          <div style={{background: 'transparent', border: '1px solid ' + styles.borderGlass, maxWidth: 'min(650px, 95vw)', width: '95%', maxHeight: '80vh', overflow: 'hidden', display: 'flex', flexDirection: 'column'}}>
            <div style={{padding: '20px 24px', borderBottom: '1px solid ' + styles.borderGlass, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px'}}>
              <div>
                <h3 style={{margin: 0, fontFamily: styles.mono, fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.purpleBright}}>Email Preview</h3>
            ...
            <div style={{padding: '16px 24px', borderTop: '1px solid ' + styles.borderGlass, display: 'flex', justifyContent: 'flex-end', gap: '10px'}}>
              <button onClick={() => setEmailPreview(null)} style={{padding: '8px 20px', background: 'transparent', border: '1px solid ' + styles.borderGlass, color: styles.textSecondary, fontFamily: styles.mono, fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>Cancel</button>
              <button onClick={confirmStateChange} className="sexy-btn" style={{padding: '8px 20px', background: emailPreview.newState === 'suspended' ? 'rgba(180,52,52,0.10)' : styles.purplePrimary, border: '1px solid ' + (emailPreview.newState === 'suspended' ? 'rgba(180,52,52,0.4)' : styles.purpleBright), color: emailPreview.newState === 'suspended' ? styles.accentRed : '#fff', fontFamily: styles.mono, fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
        ...
        <Link to="/applications" className="flex items-center gap-2 no-underline" style={{color: styles.textTertiary, fontFamily: styles.mono, fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase'}}>
        ...
        <div style={{display: 'flex', gap: '12px'}}>
          {app.state === 'pending' && (
            <button onClick={handleAdvanceToReview} className="px-4 py-2 transition-all" style={{background: 'transparent', border: '1px solid rgba(0,0,0,.09)', color: styles.accentAmber, fontFamily: styles.mono, fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
              Begin Review
            </button>
          )}
          {(app.state === 'pending' || app.state === 'under_review') && (
            <button onClick={handleApprove} className="px-4 py-2 transition-all" style={{background: 'transparent', border: '1px solid rgba(0,0,0,.09)', color: styles.accentGreen, fontFamily: styles.mono, fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
              Approve Application
            </button>
          )}
          {app.state === 'approved' && (
            <button onClick={handleScheduleTest} disabled={scheduling} className="px-4 py-2 transition-all" style={{background: styles.purplePrimary, border: `1px solid ${styles.purpleBright}`, color: '#fff', fontFamily: styles.mono, fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: scheduling ? 'wait' : 'pointer', opacity: scheduling ? 0.7 : 1}}>
              {scheduling ? 'Scheduling...' : 'Schedule CAT-72 Test'}
            </button>
          )}
          {['pending','under_review','approved','testing','conformant'].includes(app.state) && (
            <button onClick={handleSuspend} className="px-4 py-2 transition-all" style={{background: 'transparent', border: '1px solid rgba(0,0,0,0.05)', color: styles.accentRed, fontFamily: styles.mono, fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
              Suspend
            </button>
          )}
          {(app.state === 'suspended' || app.state === 'revoked') && (
            <button onClick={handleReinstate} className="px-4 py-2 transition-all" style={{background: 'transparent', border: '1px solid rgba(0,0,0,.09)', color: styles.accentGreen, fontFamily: styles.mono, fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
              Reinstate
            </button>
          )}
          {app.state === 'expired' && (
            <button onClick={handleReinstate} className="px-4 py-2 transition-all" style={{background: 'transparent', border: `1px solid ${styles.purpleBright}`, color: styles.purpleBright, fontFamily: styles.mono, fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
              Re-open
            </button>
          )}
          <button onClick={handleDeleteApplication} className="px-4 py-2 transition-all" style={{background: 'transparent', border: '1px solid rgba(0,0,0,0.05)', color: styles.accentRed, fontFamily: styles.mono, fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer'}}>
            Delete
          </button>
        </div>
        )}
      ...
      <Panel>
        <div style={{padding: '8px 0'}}>
                ...
                <React.Fragment key={stage.key}>
                  ...
                  <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', minWidth: '80px'}}>
                    <div style={{
                      ...
                      fontFamily: styles.mono, fontSize: '12px', fontWeight: 'bold',
                      background: isComplete ? 'rgba(22,135,62,0.10)' : isActive ? 'rgba(74,61,117,0.25)' : 'transparent',
                    ...
                    <span style={{fontFamily: styles.mono, fontSize: '9px', letterSpacing: '0.5px', textTransform: 'uppercase', color: isActive ? styles.purpleBright : isComplete ? styles.accentGreen : styles.textTertiary, textAlign: 'center'}}>
                      {stage.label}
                    </span>
                  </div>
                </React.Fragment>
          ...
          {isSuspended && (
            <div style={{padding: '12px 16px', background: 'transparent', border: '1px solid rgba(0,0,0,0.05)', marginBottom: '12px'}}>
              <span style={{color: styles.accentRed, fontFamily: styles.mono, fontSize: '11px'}}>⚠ SUSPENDED — This application has been suspended pending review.</span>
            </div>
          )}
      ...
      {testCreated && (
        <div className="p-4" style={{background: 'transparent', border: '1px solid rgba(0,0,0,0.05)'}}>
          <p style={{color: styles.accentGreen, fontFamily: styles.mono, fontSize: '12px'}}>
      ...
      <div>
        <p style={{fontFamily: styles.mono, fontSize: '10px', letterSpacing: '4px', textTransform: 'uppercase', color: styles.purpleBright, marginBottom: '8px'}}>Application {app.application_number}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Panel>
          <h2 style={{fontFamily: styles.mono, fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '16px'}}>Organization</h2>
        ...
        <Panel>
          <h2 style={{fontFamily: styles.mono, fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '16px'}}>Status</h2>
      ...
      if (newState === 'approved') {
        try {
          ...
          console.log('Auto-provision note:', provErr.message);
              ...
              className="px-3 py-2"
              style={{background: 'transparent', border: `1px solid ${styles.borderGlass}`, color: styles.textPrimary, fontFamily: styles.mono, fontSize: '11px'}}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Panel>
          <h2 style={{fontFamily: styles.mono, fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '16px'}}>System Details</h2>
        ...
        <Panel>
          <h2 style={{fontFamily: styles.mono, fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '16px'}}>Safety Boundaries & Operational Limits</h2>
      ...
      {history.length > 0 && (
        <Panel>
          <h2 style={{fontFamily: styles.mono, fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '20px'}}>State Change History</h2>
            ...
            {history.map((entry, i) => {
              const actionColors = {
                ...
                state_changed: (() => {
                  ...
                  if (ns === 'suspended' || ns === 'revoked') return styles.accentRed;
              ...
              return (
                <div key={i} style={{position: 'relative', paddingBottom: i < history.length - 1 ? '20px' : '0', marginBottom: i < history.length - 1 ? '0' : '0'}}>
                  ...
                  <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px'}}>
                    <div>
                      <span style={{fontFamily: styles.mono, fontSize: '12px', fontWeight: 500, color: color, textTransform: 'uppercase', letterSpacing: '0.5px'}}>{stateLabel}</span>
                      {fromLabel && entry.action !== 'submitted' && (
                        <span style={{fontFamily: styles.mono, fontSize: '10px', color: styles.textTertiary, marginLeft: '8px'}}>from {fromLabel}</span>
                      )}
                    ...
                    <div style={{textAlign: 'right', flexShrink: 0}}>
                      {ts && (
                        <>
                          <div style={{fontFamily: styles.mono, fontSize: '11px', color: styles.textTertiary}}>{ts.toLocaleDateString()}</div>
                          <div style={{fontFamily: styles.mono, fontSize: '10px', color: styles.textTertiary}}>{ts.toLocaleTimeString()}</div>
                        </>
                      )}
                    </div>
                  </div>
      ...
      <Panel>
        <h2 style={{fontFamily: styles.mono, fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '16px'}}>ODD Specification</h2>
      ...
      {/* Comments Thread */}
      <Panel>
        <h2 style={{fontFamily: styles.mono, fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '16px'}}>Comments & Notes</h2>
        ...
        <div style={{marginBottom: comments.length > 0 ? '20px' : '0'}}>
          <textarea
            ...
            style={{background: 'transparent', border: `1px solid ${styles.borderGlass}`, color: styles.textPrimary, fontSize: '13px', fontFamily: 'inherit', transition: 'border-color 0.2s'}}
            ...
            <div style={{display: 'flex', alignItems: 'center', gap: '8px'}}>
              {user?.role === 'admin' && (
                <label style={{display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer'}}>
                  ...
                  <span style={{fontFamily: styles.mono, fontSize: '10px', color: styles.textTertiary, letterSpacing: '0.5px', textTransform: 'uppercase'}}>Internal only</span>
                </label>
              )}
            </div>
            <button
              ...
              className="px-4 py-2"
              style={{background: newComment.trim() ? styles.purplePrimary : 'rgba(0,0,0,0.025)', border: `1px solid ${newComment.trim() ? styles.purpleBright : styles.borderGlass}`, color: newComment.trim() ? '#fff' : styles.textTertiary, fontFamily: styles.mono, fontSize: '11px', letterSpacing: '1px', textTransform: 'uppercase', cursor: newComment.trim() ? 'pointer' : 'default', opacity: postingComment ? 0.6 : 1}}
            >
        ...
        {comments.length > 0 && (
          <div style={{borderTop: `1px solid ${styles.borderGlass}`, paddingTop: '16px'}}>
            {comments.map((c) => (
              <div key={c.id} style={{padding: '12px 0', borderBottom: `1px solid rgba(0,0,0,0.05)`}}>
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px', marginBottom: '6px'}}>
                  <div style={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                    <span style={{fontFamily: styles.mono, fontSize: '12px', color: c.user_role === 'admin' ? styles.purpleBright : styles.textSecondary, fontWeight: 500}}>{c.user_email}</span>
                    {c.user_role === 'admin' && <span style={{fontFamily: styles.mono, fontSize: '9px', padding: '2px 6px', background: 'transparent', color: styles.purpleBright, textTransform: 'uppercase', letterSpacing: '0.5px'}}>Admin</span>}
                    {c.is_internal && <span style={{fontFamily: styles.mono, fontSize: '9px', padding: '2px 6px', background: 'transparent', color: styles.accentAmber, textTransform: 'uppercase', letterSpacing: '0.5px'}}>Internal</span>}
                  </div>
                  <div style={{display: 'flex', alignItems: 'center', gap: '8px'}}>
                    <span style={{fontFamily: styles.mono, fontSize: '10px', color: styles.textTertiary}}>{c.created_at ? new Date(c.created_at).toLocaleString() : ''}</span>
      ...
      {app.notes && (
        <Panel>
          <h2 style={{fontFamily: styles.mono, fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', color: styles.textTertiary, marginBottom: '16px'}}>Applicant Notes</h2>
