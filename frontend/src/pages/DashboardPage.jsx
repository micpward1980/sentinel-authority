            ...
            {needsAction.slice(0, 5).map(app => (
              <div key={app.id} style={{padding: '14px 16px', background: 'transparent', border: `1px solid ${styles.borderGlass}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px'}}>
                <div style={{display: 'flex', alignItems: 'center', gap: '16px'}}>
                  ...
                  <span style={{color: styles.accentAmber, fontFamily: styles.mono, fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase'}}>{app.state?.replace('_', ' ')}</span>
                </div>
                <div style={{display: 'flex', gap: '8px'}}>
                  {app.state === 'pending' && (
                    <button onClick={() => handleQuickAdvance(app.id, 'under_review', `Begin review for ${app.system_name}`)} style={{background: 'transparent', border: '1px solid ' + styles.borderSubtle, color: styles.accentAmber, fontFamily: styles.mono, fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer', padding: '4px 10px'}}>Begin Review</button>
                  )}
                  <button onClick={() => handleQuickAdvance(app.id, 'approved', `Approve ${app.system_name}`)} style={{background: 'transparent', border: '1px solid ' + styles.borderSubtle, color: styles.accentGreen, fontFamily: styles.mono, fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer', padding: '4px 10px'}}>Approve</button>
                  <button onClick={() => handleQuickAdvance(app.id, 'suspended', `Suspend ${app.system_name}`)} style={{background: 'transparent', border: '1px solid ' + styles.borderSubtle, color: styles.accentRed, fontFamily: styles.mono, fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer', padding: '4px 10px'}}>Suspend</button>
                  <Link to={`/applications/${app.id}`} style={{background: 'transparent', border: `1px solid ${styles.borderGlass}`, color: styles.purpleBright, fontFamily: styles.mono, fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase', textDecoration: 'none', padding: '4px 10px'}}>View</Link>
                </div>
              </div>
            ))}
          </div>
              ...
              <thead>
                <tr style={{borderBottom: `1px solid ${styles.borderGlass}`}}>
                  {['Certificate', 'System', 'Status', 'Issued', 'Expires'].map(h => (
                    <th key={h} style={{padding: '12px 16px', textAlign: 'left', fontFamily: styles.mono, fontSize: '9px', letterSpacing: '1.5px', textTransform: 'uppercase', color: styles.textTertiary, fontWeight: 400}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
