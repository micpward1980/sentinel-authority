    ...
    <div className="space-y-6">
      ...
      <Panel>
        <div style={{display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap'}}>
          <select value={actionFilter} onChange={e => { setActionFilter(e.target.value); setPage(0); }} style={{background: '#2a2f3d', border: `1px solid ${'rgba(255,255,255,.07)'}`, padding: '8px 12px', color: 'rgba(255,255,255,.94)', fontSize: '12px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", minWidth: '180px'}}>
            <option value="">All Actions</option>
            {actions.map(a => <option key={a} value={a}>{a}</option>)}
          </select>
          <select value={resourceFilter} onChange={e => { setResourceFilter(e.target.value); setPage(0); }} style={{background: '#2a2f3d', border: `1px solid ${'rgba(255,255,255,.07)'}`, padding: '8px 12px', color: 'rgba(255,255,255,.94)', fontSize: '12px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", minWidth: '150px'}}>
            <option value="">All Resources</option>
            {resourceTypes.map(t => <option key={t} value={t}>{t}</option>)}
          </select>
          <input value={emailFilter} onChange={e => { setEmailFilter(e.target.value); setPage(0); }} placeholder="Filter by email..." style={{background: '#2a2f3d', border: `1px solid ${'rgba(255,255,255,.07)'}`, padding: '8px 12px', color: 'rgba(255,255,255,.94)', fontSize: '12px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", flex: 1, minWidth: '150px'}} />
          {['7', '30', '90', 'all'].map(r => (
          ...
          <input type="date" value={dateFrom} onChange={e => { setDateFrom(e.target.value); setPage(0); setQuickRange(''); }} style={{background: '#2a2f3d', border: `1px solid ${'rgba(255,255,255,.07)'}`, padding: '8px 12px', color: 'rgba(255,255,255,.94)', fontSize: '12px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", colorScheme: 'dark'}} />
          <span style={{fontSize: '11px', color: 'rgba(255,255,255,.50)'}}>to</span>
          <input type="date" value={dateTo} onChange={e => { setDateTo(e.target.value); setPage(0); setQuickRange(''); }} style={{background: '#2a2f3d', border: `1px solid ${'rgba(255,255,255,.07)'}`, padding: '8px 12px', color: 'rgba(255,255,255,.94)', fontSize: '12px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", colorScheme: 'dark'}} />
          <span style={{fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '11px', color: 'rgba(255,255,255,.50)'}}>{total.toLocaleString()} entries</span>
          <button onClick={exportCSV} style={{padding: '6px 14px', background: 'rgba(157,140,207,0.12)', border: '1px solid ' + 'rgba(157,140,207,0.3)', color: '#a896d6', fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '10px', cursor: 'pointer', letterSpacing: '1px', textTransform: 'uppercase'}}>Export CSV</button>
          <button onClick={verifyIntegrity} disabled={verifying} style={{padding: '6px 14px', background: verifyResult ? (verifyResult.integrity === 'passed' ? 'rgba(92,214,133,0.12)' : 'rgba(214,92,92,0.12)') : 'rgba(157,140,207,0.12)', border: '1px solid ' + (verifyResult ? (verifyResult.integrity === 'passed' ? 'rgba(92,214,133,0.3)' : 'rgba(214,92,92,0.3)') : 'rgba(157,140,207,0.3)'), color: verifyResult ? (verifyResult.integrity === 'passed' ? '#5CD685' : '#D65C5C') : '#a896d6', fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '10px', cursor: verifying ? 'wait' : 'pointer', letterSpacing: '1px', textTransform: 'uppercase'}}>
      ...
      <Panel noPad>
        {loading ? (
          ...
          <div style={{padding: 'clamp(16px, 4vw, 40px)', textAlign: 'center', color: 'rgba(255,255,255,.50)'}}>No audit log entries found</div>
        ) : (
          <div className='table-scroll' style={{overflowX: 'auto', WebkitOverflowScrolling: 'touch'}}>
            <table style={{width: '100%', borderCollapse: 'collapse'}}>
              <thead>
                <tr style={{background: 'transparent'}}>
                  <th style={{padding: '12px 16px', textAlign: 'left', fontSize: '10px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", textTransform: 'uppercase', letterSpacing: '1px', color: 'rgba(255,255,255,.50)'}}>Timestamp</th>
                  <th style={{padding: '12px 16px', textAlign: 'left', fontSize: '10px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", textTransform: 'uppercase', letterSpacing: '1px', color: 'rgba(255,255,255,.50)'}}>User</th>
                  <th style={{padding: '12px 16px', textAlign: 'left', fontSize: '10px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", textTransform: 'uppercase', letterSpacing: '1px', color: 'rgba(255,255,255,.50)'}}>Action</th>
                  <th style={{padding: '12px 16px', textAlign: 'left', fontSize: '10px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", textTransform: 'uppercase', letterSpacing: '1px', color: 'rgba(255,255,255,.50)'}}>Resource</th>
                  <th style={{padding: '12px 16px', textAlign: 'left', fontSize: '10px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", textTransform: 'uppercase', letterSpacing: '1px', color: 'rgba(255,255,255,.50)'}}>Details</th>
                  <th style={{padding: '12px 16px', textAlign: 'left', fontSize: '10px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", textTransform: 'uppercase', letterSpacing: '1px', color: 'rgba(255,255,255,.50)'}}>Hash</th>
                </tr>
              </thead>
              <tbody>
                {logs.map(log => (
                  <tr key={log.id} style={{borderBottom: `1px solid ${'rgba(255,255,255,.04)'}`}}>
                    <td style={{padding: '12px 16px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '11px', color: 'rgba(255,255,255,.78)', whiteSpace: 'nowrap'}}>{log.timestamp ? new Date(log.timestamp).toLocaleString() : '-'}</td>
                    <td style={{padding: '12px 16px', fontSize: '13px', color: 'rgba(255,255,255,.94)'}}>{log.user_email || '-'}</td>
                    ...
                    <td style={{padding: '12px 16px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '11px', color: 'rgba(255,255,255,.50)'}}>
                    ...
                    <td style={{padding: '12px 16px', fontFamily: "Consolas, 'IBM Plex Mono', monospace", fontSize: '10px', color: 'rgba(255,255,255,.50)', letterSpacing: '0.5px'}}>{log.log_hash || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
