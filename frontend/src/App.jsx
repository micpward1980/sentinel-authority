...
import { Wifi, Bell, Settings, FileText, Activity, Award, Users, Home, LogOut, Menu, X, CheckCircle, AlertTriangle, Clock, Search, Plus, ArrowLeft, ExternalLink, Shield, Download, RefreshCw, Eye, EyeOff, BookOpen, AlertCircle, Cpu} from 'lucide-react';
...
function Layout({ children }) {
  ...
  useEffect(() => {
    if (user) {
      ...
      api.get('/api/users/notifications').then(res => { setNotifs(res.data.notifications || []); setUnreadCount(res.data.unread_count || 0); }).catch(() => {});
    }
  }, [user]);
  ...
  useEffect(() => {
    if (!user) return;
    const iv = setInterval(() => api.get('/api/users/notifications').then(r => { setNotifs(r.data.notifications || []); setUnreadCount(r.data.unread_count || 0); }).catch(() => {}), 60000);
          ...
          {data.length === 0 ? (
            <tr><td colSpan={columns.length} style={{padding: '48px 24px', textAlign: 'center'}}>
              <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px'}}>
                <div style={{width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(91,75,138,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center'}}><Search size={18} style={{color: styles.textTertiary, opacity: 0.5}} /></div>
