...
export const SYSTEM_TYPES = {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  hiring_screening_ai: {
    label: 'Hiring / HR Screening AI',
    domain: 'government',
    template: {
      numeric: [
        { name: 'bias_disparity_ratio', min: 0, max: 1.25, unit: 'ratio', tolerance: 0.05 },
        { name: 'confidence_threshold', min: 0.70, max: null, unit: 'score', tolerance: 0.03 },
        { name: 'human_review_ratio', min: 0.20, max: null, unit: 'ratio', tolerance: 0.05 },
        { name: 'false_negative_rate', min: 0, max: 0.15, unit: 'rate', tolerance: 0.02 },
        { name: 'consecutive_autonomous_max', min: 0, max: 50, unit: 'decisions', tolerance: 5 },
        { name: 'data_staleness_max', min: 0, max: 86400, unit: 'sec', tolerance: 3600 },
        { name: 'explainability_score_min', min: 0.60, max: null, unit: 'score', tolerance: 0.05 },
        { name: 'decision_rate', min: 0, max: 200, unit: 'decisions/hr', tolerance: 20 },
      ],
      geo: { type: 'logical', description: 'Approved job categories and jurisdictions — applicable labor law regions only' },
      time: { operating_hours: '06:00-22:00', operating_days: [0,1,2,3,4], timezone: 'ops_local' },
      states: { allowed: ['screening','scoring','shortlisting','human_review','archived'], forbidden: ['auto_reject_protected_class','bias_threshold_exceeded','unexplainable_decision'] },
      odd_description: 'Resume screening and candidate scoring system. System provides ranked shortlists for human recruiter review. Cannot autonomously reject candidates — all rejections require human confirmation. EEOC/OFCCP compliant. Disparate impact monitored continuously.',
      safety: { violation_action: 'block', fail_closed: true, human_review_required: true, audit_trail: true },
    },
  },
  sentencing_recommendation_ai: {
    label: 'Sentencing / Parole Recommendation AI',
    domain: 'government',
    template: {
      numeric: [
        { name: 'bias_disparity_ratio', min: 0, max: 1.10, unit: 'ratio', tolerance: 0.02 },
        { name: 'confidence_threshold', min: 0.85, max: null, unit: 'score', tolerance: 0.02 },
        { name: 'human_review_ratio', min: 1.0, max: null, unit: 'ratio', tolerance: 0 },
        { name: 'false_positive_rate', min: 0, max: 0.05, unit: 'rate', tolerance: 0.01 },
        { name: 'false_negative_rate', min: 0, max: 0.05, unit: 'rate', tolerance: 0.01 },
        { name: 'explainability_score_min', min: 0.80, max: null, unit: 'score', tolerance: 0.03 },
        { name: 'model_drift_threshold', min: 0, max: 0.05, unit: 'delta', tolerance: 0.01 },
        { name: 'data_staleness_max', min: 0, max: 3600, unit: 'sec', tolerance: 300 },
      ],
      geo: { type: 'logical', description: 'Authorized court jurisdictions — applicable sentencing guidelines and statutory frameworks' },
      time: { operating_hours: '07:00-19:00', operating_days: [0,1,2,3,4], timezone: 'court_local' },
      states: { allowed: ['assessment','scoring','report_generation','judicial_review','sealed'], forbidden: ['auto_sentence','bias_detected','unexplainable_recommendation','stale_data'] },
      odd_description: 'Risk assessment and sentencing recommendation advisory system. ALL recommendations require judicial review — system has ZERO autonomous sentencing authority. Continuous bias auditing with demographic parity monitoring. Court-approved validation required annually.',
      safety: { violation_action: 'block', fail_closed: true, human_review_required: true, judicial_override: true, audit_trail: true },
    },
  },
  child_welfare_ai: {
    label: 'Child Welfare Risk Scoring AI',
    domain: 'government',
    template: {
      numeric: [
        { name: 'bias_disparity_ratio', min: 0, max: 1.15, unit: 'ratio', tolerance: 0.03 },
        { name: 'confidence_threshold', min: 0.80, max: null, unit: 'score', tolerance: 0.03 },
        { name: 'human_review_ratio', min: 1.0, max: null, unit: 'ratio', tolerance: 0 },
        { name: 'false_negative_rate', min: 0, max: 0.03, unit: 'rate', tolerance: 0.005 },
        { name: 'consecutive_autonomous_max', min: 0, max: 0, unit: 'decisions', tolerance: 0 },
        { name: 'escalation_timeout_sec', min: 0, max: 3600, unit: 'sec', tolerance: 300 },
        { name: 'explainability_score_min', min: 0.75, max: null, unit: 'score', tolerance: 0.05 },
        { name: 'data_staleness_max', min: 0, max: 7200, unit: 'sec', tolerance: 600 },
      ],
      geo: { type: 'logical', description: 'County/state child protective services jurisdiction — applicable mandated reporting laws' },
      time: { operating_hours: '00:00-23:59', operating_days: [0,1,2,3,4,5,6], timezone: 'agency_local' },
      states: { allowed: ['intake_screening','risk_scoring','caseworker_review','supervisor_review','closed'], forbidden: ['auto_close_high_risk','auto_removal_recommendation','caseworker_bypass'] },
      odd_description: 'Child safety risk scoring for intake screening. System flags cases for caseworker assessment — ZERO autonomous case closure or removal decisions. Every scored case reviewed by human caseworker. Continuous bias monitoring across demographics.',
      safety: { violation_action: 'block', fail_closed: true, human_review_required: true, supervisor_escalation: true, audit_trail: true },
    },
  },
  benefits_eligibility_ai: {
    label: 'Benefits / Disability Eligibility AI',
    domain: 'government',
    template: {
      numeric: [
        { name: 'bias_disparity_ratio', min: 0, max: 1.20, unit: 'ratio', tolerance: 0.03 },
        { name: 'confidence_threshold', min: 0.80, max: null, unit: 'score', tolerance: 0.03 },
        { name: 'human_review_ratio', min: 0.30, max: null, unit: 'ratio', tolerance: 0.05 },
        { name: 'false_negative_rate', min: 0, max: 0.05, unit: 'rate', tolerance: 0.01 },
        { name: 'decision_rate', min: 0, max: 500, unit: 'decisions/hr', tolerance: 50 },
        { name: 'escalation_timeout_sec', min: 0, max: 7200, unit: 'sec', tolerance: 600 },
        { name: 'explainability_score_min', min: 0.70, max: null, unit: 'score', tolerance: 0.05 },
        { name: 'data_staleness_max', min: 0, max: 86400, unit: 'sec', tolerance: 3600 },
      ],
      geo: { type: 'logical', description: 'Applicable benefit programs and jurisdictional eligibility rules' },
      time: { operating_hours: '06:00-22:00', operating_days: [0,1,2,3,4,5,6], timezone: 'ops_local' },
      states: { allowed: ['intake','document_review','scoring','human_review','approved','denied','appeal'], forbidden: ['auto_deny_without_review','stale_data_decision','unexplainable_denial'] },
      odd_description: 'Automated eligibility screening for government benefit programs. System pre-screens applications and flags for human adjudicator review. Denials require human confirmation and written explanation. APA/due process compliant.',
      safety: { violation_action: 'block', fail_closed: true, human_review_required: true, appeal_path_required: true, audit_trail: true },
    },
  },
  clinical_trial_matching_ai: {
    label: 'Clinical Trial Matching AI',
    domain: 'medical',
    template: {
      numeric: [
        { name: 'confidence_threshold', min: 0.80, max: null, unit: 'score', tolerance: 0.03 },
        { name: 'false_positive_rate', min: 0, max: 0.10, unit: 'rate', tolerance: 0.02 },
        { name: 'false_negative_rate', min: 0, max: 0.08, unit: 'rate', tolerance: 0.02 },
        { name: 'human_review_ratio', min: 1.0, max: null, unit: 'ratio', tolerance: 0 },
        { name: 'consent_verification_required', min: 1, max: 1, unit: 'bool', tolerance: 0 },
        { name: 'data_staleness_max', min: 0, max: 86400, unit: 'sec', tolerance: 3600 },
        { name: 'model_drift_threshold', min: 0, max: 0.08, unit: 'delta', tolerance: 0.02 },
        { name: 'decision_rate', min: 0, max: 100, unit: 'matches/hr', tolerance: 10 },
      ],
      geo: { type: 'logical', description: 'Approved clinical sites and IRB-approved trial protocols — patient catchment areas' },
      time: { operating_hours: '00:00-23:59', operating_days: [0,1,2,3,4,5,6], timezone: 'facility_local' },
      states: { allowed: ['screening','matching','clinician_review','patient_consent','enrolled','ineligible'], forbidden: ['auto_enroll','consent_bypassed','exclusion_criteria_ignored'] },
      odd_description: 'Patient-trial matching system using EHR data and trial protocols. System suggests matches for clinician review — cannot auto-enroll. Patient consent verified independently. HIPAA compliant, IRB-approved protocols only.',
      safety: { violation_action: 'block', fail_closed: true, clinician_review_required: true, consent_required: true, audit_trail: true },
    },
  },
  surgical_planning_ai: {
    label: 'Surgical Planning / Guidance AI',
    domain: 'medical',
    template: {
      numeric: [
        { name: 'confidence_threshold', min: 0.90, max: null, unit: 'score', tolerance: 0.02 },
        { name: 'max_planning_latency', min: 0, max: 30, unit: 'sec', tolerance: 5 },
        { name: 'positional_accuracy', min: 0, max: 2.0, unit: 'mm', tolerance: 0.5 },
        { name: 'false_positive_rate', min: 0, max: 0.02, unit: 'rate', tolerance: 0.005 },
        { name: 'false_negative_rate', min: 0, max: 0.02, unit: 'rate', tolerance: 0.005 },
        { name: 'human_review_ratio', min: 1.0, max: null, unit: 'ratio', tolerance: 0 },
        { name: 'model_drift_threshold', min: 0, max: 0.03, unit: 'delta', tolerance: 0.01 },
        { name: 'data_staleness_max', min: 0, max: 300, unit: 'sec', tolerance: 30 },
      ],
      geo: { type: 'logical', description: 'Approved surgical suites and imaging systems — validated instrument sets only' },
      time: { operating_hours: '06:00-22:00', operating_days: [0,1,2,3,4,5,6], timezone: 'facility_local' },
      states: { allowed: ['imaging','planning','surgeon_review','intraop_guidance','post_op_analysis'], forbidden: ['autonomous_incision','surgeon_override_bypassed','uncalibrated_guidance','stale_imaging'] },
      odd_description: 'AI-assisted surgical planning and intraoperative guidance. System provides 3D planning, trajectory optimization, and real-time guidance overlays. Surgeon maintains full manual authority at all times. FDA 510(k)/De Novo cleared.',
      safety: { violation_action: 'block', fail_closed: true, surgeon_authority: true, calibration_required: true, audit_trail: true },
    },
  },
  drug_interaction_ai: {
    label: 'Drug Interaction Checker AI',
    domain: 'medical',
    template: {
      numeric: [
        { name: 'confidence_threshold', min: 0.85, max: null, unit: 'score', tolerance: 0.02 },
        { name: 'false_negative_rate', min: 0, max: 0.01, unit: 'rate', tolerance: 0.002 },
        { name: 'max_response_latency', min: 0, max: 2000, unit: 'ms', tolerance: 200 },
        { name: 'data_staleness_max', min: 0, max: 86400, unit: 'sec', tolerance: 3600 },
        { name: 'model_drift_threshold', min: 0, max: 0.05, unit: 'delta', tolerance: 0.01 },
        { name: 'decision_rate', min: 0, max: 10000, unit: 'checks/hr', tolerance: 1000 },
        { name: 'database_coverage_min', min: 0.95, max: null, unit: 'ratio', tolerance: 0.01 },
      ],
      geo: { type: 'logical', description: 'Connected EHR/pharmacy systems — approved drug databases and formularies' },
      time: { operating_hours: '00:00-23:59', operating_days: [0,1,2,3,4,5,6], timezone: 'facility_local' },
      states: { allowed: ['active','checking','alert_generated','pharmacist_review','overridden_with_reason'], forbidden: ['interaction_suppressed','database_stale','unchecked_dispense'] },
      odd_description: 'Real-time drug-drug, drug-allergy, and drug-condition interaction checking integrated with EHR/pharmacy workflow. System blocks dispensing on critical interactions until pharmacist review. Cannot be silently overridden.',
      safety: { violation_action: 'block', fail_closed: true, pharmacist_review: true, override_requires_reason: true, audit_trail: true },
    },
  },
  mental_health_risk_ai: {
    label: 'Mental Health Risk Assessment AI',
    domain: 'medical',
    template: {
      numeric: [
        { name: 'confidence_threshold', min: 0.75, max: null, unit: 'score', tolerance: 0.03 },
        { name: 'false_negative_rate', min: 0, max: 0.02, unit: 'rate', tolerance: 0.005 },
        { name: 'human_review_ratio', min: 1.0, max: null, unit: 'ratio', tolerance: 0 },
        { name: 'escalation_timeout_sec', min: 0, max: 900, unit: 'sec', tolerance: 60 },
        { name: 'consecutive_autonomous_max', min: 0, max: 0, unit: 'decisions', tolerance: 0 },
        { name: 'bias_disparity_ratio', min: 0, max: 1.20, unit: 'ratio', tolerance: 0.05 },
        { name: 'data_staleness_max', min: 0, max: 3600, unit: 'sec', tolerance: 300 },
        { name: 'explainability_score_min', min: 0.70, max: null, unit: 'score', tolerance: 0.05 },
      ],
      geo: { type: 'logical', description: 'Approved clinical settings — licensed provider jurisdictions and telehealth-eligible states' },
      time: { operating_hours: '00:00-23:59', operating_days: [0,1,2,3,4,5,6], timezone: 'facility_local' },
      states: { allowed: ['screening','risk_scoring','clinician_alert','clinician_review','safety_plan','crisis_escalation'], forbidden: ['auto_discharge_high_risk','suppress_self_harm_flag','clinician_bypass','stale_assessment'] },
      odd_description: 'Suicide/self-harm risk screening and mental health acuity scoring. System flags at-risk patients for immediate clinician review — ZERO autonomous clinical decisions. Crisis-level flags trigger immediate escalation with timeout enforcement.',
      safety: { violation_action: 'block', fail_closed: true, clinician_review_required: true, crisis_escalation: true, audit_trail: true },
    },
  },
  ...
  // ══════════════════════════════════════════════════════════════════════════
  water_treatment_ai: {
    label: 'Water Treatment Control AI',
    domain: 'critical_infra',
    template: {
      numeric: [
        { name: 'ph_range', min: 6.5, max: 8.5, unit: 'pH', tolerance: 0.2 },
        { name: 'chlorine_residual', min: 0.2, max: 4.0, unit: 'mg/L', tolerance: 0.1 },
        { name: 'turbidity_max', min: 0, max: 1.0, unit: 'NTU', tolerance: 0.1 },
        { name: 'flow_rate', min: 0, max: 50000, unit: 'gal/min', tolerance: 500 },
        { name: 'confidence_threshold', min: 0.90, max: null, unit: 'score', tolerance: 0.02 },
        { name: 'human_review_ratio', min: 0.10, max: null, unit: 'ratio', tolerance: 0.02 },
        { name: 'decision_rate', min: 0, max: 60, unit: 'adjustments/hr', tolerance: 5 },
        { name: 'data_staleness_max', min: 0, max: 60, unit: 'sec', tolerance: 10 },
      ],
      geo: { type: 'polygon', description: 'Treatment plant facility boundary — intake, treatment, and distribution control zones' },
      time: { operating_hours: '00:00-23:59', operating_days: [0,1,2,3,4,5,6], timezone: 'facility_local' },
      states: { allowed: ['normal_operation','adjustment','flushing','backwash','operator_review','maintenance'], forbidden: ['chemical_limit_exceeded','sensor_fault_unacknowledged','operator_lockout_bypassed'] },
      odd_description: 'AI-controlled water treatment process optimization. System adjusts chemical dosing, filtration, and flow within EPA-mandated Safe Drinking Water Act parameters. Operator can override at any time. Critical parameter changes require operator confirmation.',
      safety: { violation_action: 'block', fail_closed: true, operator_override: true, epa_compliance: true, audit_trail: true },
    },
  },
  air_traffic_ai: {
    label: 'Air Traffic Management AI',
    domain: 'critical_infra',
    template: {
      numeric: [
        { name: 'min_separation_horizontal', min: 3, max: null, unit: 'nm', tolerance: 0.5 },
        { name: 'min_separation_vertical', min: 1000, max: null, unit: 'ft', tolerance: 100 },
        { name: 'max_response_latency', min: 0, max: 500, unit: 'ms', tolerance: 50 },
        { name: 'confidence_threshold', min: 0.95, max: null, unit: 'score', tolerance: 0.01 },
        { name: 'false_negative_rate', min: 0, max: 0.001, unit: 'rate', tolerance: 0.0005 },
        { name: 'human_review_ratio', min: 1.0, max: null, unit: 'ratio', tolerance: 0 },
        { name: 'data_staleness_max', min: 0, max: 5, unit: 'sec', tolerance: 1 },
        { name: 'decision_rate', min: 0, max: 300, unit: 'advisories/hr', tolerance: 30 },
      ],
      geo: { type: 'polygon_3d', description: 'Assigned airspace sectors — terminal, en-route, and transition zones with altitude blocks' },
      time: { operating_hours: '00:00-23:59', operating_days: [0,1,2,3,4,5,6], timezone: 'UTC' },
      states: { allowed: ['monitoring','advisory','conflict_detection','resolution_advisory','controller_review','emergency'], forbidden: ['autonomous_reroute','controller_override_bypassed','radar_contact_lost_unacknowledged'] },
      odd_description: 'AI advisory system for air traffic conflict detection and resolution. System provides separation advisories to human controllers — ZERO autonomous authority over aircraft routing or clearances. FAA NextGen/EUROCONTROL compliant. Controller maintains full authority.',
      safety: { violation_action: 'block', fail_closed: true, controller_authority: true, tcas_integration: true, audit_trail: true },
    },
  },
  railway_signaling_ai: {
    label: 'Railway Signaling / Train Control AI',
    domain: 'critical_infra',
    template: {
      numeric: [
        { name: 'min_headway', min: 90, max: null, unit: 'sec', tolerance: 10 },
        { name: 'max_speed', min: 0, max: 300, unit: 'km/h', tolerance: 5 },
        { name: 'braking_distance_margin', min: 50, max: null, unit: 'meters', tolerance: 10 },
        { name: 'max_response_latency', min: 0, max: 200, unit: 'ms', tolerance: 20 },
        { name: 'confidence_threshold', min: 0.99, max: null, unit: 'score', tolerance: 0.002 },
        { name: 'false_negative_rate', min: 0, max: 0.0001, unit: 'rate', tolerance: 0.00005 },
        { name: 'data_staleness_max', min: 0, max: 2, unit: 'sec', tolerance: 0.5 },
        { name: 'model_drift_threshold', min: 0, max: 0.01, unit: 'delta', tolerance: 0.002 },
      ],
      geo: { type: 'polygon', description: 'Track segments, interlocking zones, platform areas, and level crossings under AI control' },
      time: { operating_hours: '00:00-23:59', operating_days: [0,1,2,3,4,5,6], timezone: 'network_local' },
      states: { allowed: ['green','caution','restrictive','stop','emergency_brake','maintenance_mode'], forbidden: ['signal_conflict','interlocking_fault_unacknowledged','occupied_block_cleared'] },
      odd_description: 'AI-enhanced positive train control and signaling. System enforces movement authorities, speed restrictions, and safe separation. Fail-safe defaults to most restrictive signal aspect. ERTMS/PTC compliant. Human dispatcher maintains route authority.',
      safety: { violation_action: 'block', fail_closed: true, fail_safe_restrictive: true, dispatcher_authority: true, audit_trail: true },
    },
  },
  power_grid_ai: {
    label: 'Power Grid Load Balancing AI',
    domain: 'critical_infra',
    template: {
      numeric: [
        { name: 'frequency_range', min: 59.95, max: 60.05, unit: 'Hz', tolerance: 0.01 },
        { name: 'voltage_range_pu', min: 0.95, max: 1.05, unit: 'p.u.', tolerance: 0.005 },
        { name: 'max_ramp_rate', min: 0, max: 500, unit: 'MW/min', tolerance: 50 },
        { name: 'reserve_margin_min', min: 0.15, max: null, unit: 'ratio', tolerance: 0.02 },
        { name: 'confidence_threshold', min: 0.90, max: null, unit: 'score', tolerance: 0.02 },
        { name: 'decision_rate', min: 0, max: 120, unit: 'dispatches/hr', tolerance: 10 },
        { name: 'data_staleness_max', min: 0, max: 4, unit: 'sec', tolerance: 1 },
        { name: 'human_review_ratio', min: 0.05, max: null, unit: 'ratio', tolerance: 0.02 },
      ],
      geo: { type: 'logical', description: 'Balancing authority area — interconnected generation, transmission, and load zones' },
      time: { operating_hours: '00:00-23:59', operating_days: [0,1,2,3,4,5,6], timezone: 'grid_local' },
      states: { allowed: ['normal','economic_dispatch','contingency','emergency','restoration','islanded'], forbidden: ['frequency_out_of_band','cascading_trip','load_shed_unauthorized','blackstart_without_approval'] },
      odd_description: 'AI-optimized economic dispatch and load balancing for electric grid. System optimizes generation scheduling within NERC reliability standards. Emergency load shedding requires operator authorization. NERC CIP compliant.',
      safety: { violation_action: 'block', fail_closed: true, operator_authority: true, nerc_compliance: true, audit_trail: true },
    },
  },
  ...
  // ══════════════════════════════════════════════════════════════════════════
  robo_advisor_ai: {
    label: 'Robo-Advisor / Wealth Management AI',
    domain: 'financial',
    template: {
      numeric: [
        { name: 'max_single_trade_pct', min: 0, max: 5, unit: '%_portfolio', tolerance: 0.5 },
        { name: 'max_daily_rebalance_pct', min: 0, max: 15, unit: '%_portfolio', tolerance: 2 },
        { name: 'concentration_limit', min: 0, max: 25, unit: '%_single_asset', tolerance: 2 },
        { name: 'confidence_threshold', min: 0.80, max: null, unit: 'score', tolerance: 0.03 },
        { name: 'human_review_ratio', min: 0.05, max: null, unit: 'ratio', tolerance: 0.02 },
        { name: 'model_drift_threshold', min: 0, max: 0.10, unit: 'delta', tolerance: 0.02 },
        { name: 'decision_rate', min: 0, max: 1000, unit: 'trades/hr', tolerance: 100 },
        { name: 'data_staleness_max', min: 0, max: 300, unit: 'sec', tolerance: 30 },
      ],
      geo: { type: 'logical', description: 'Approved exchanges, asset classes, and jurisdictions — SEC/FINRA registered instruments only' },
      time: { operating_hours: '04:00-20:00', operating_days: [0,1,2,3,4], timezone: 'America/New_York' },
      states: { allowed: ['monitoring','rebalancing','tax_loss_harvesting','client_review','compliance_check'], forbidden: ['concentration_exceeded','suitability_mismatch','unauthorized_asset_class'] },
      odd_description: 'Automated portfolio management and rebalancing. System executes within client-approved investment policy statements. Trades exceeding thresholds require advisor review. SEC/FINRA/DOL fiduciary compliant.',
      safety: { violation_action: 'block', fail_closed: true, advisor_review_threshold: true, suitability_check: true, audit_trail: true },
    },
  },
  property_valuation_ai: {
    label: 'Automated Property Valuation (AVM)',
    domain: 'financial',
    template: {
      numeric: [
        { name: 'confidence_threshold', min: 0.75, max: null, unit: 'score', tolerance: 0.03 },
        { name: 'max_valuation_variance', min: 0, max: 15, unit: '%', tolerance: 2 },
        { name: 'bias_disparity_ratio', min: 0, max: 1.15, unit: 'ratio', tolerance: 0.03 },
        { name: 'data_staleness_max', min: 0, max: 604800, unit: 'sec', tolerance: 86400 },
        { name: 'model_drift_threshold', min: 0, max: 0.08, unit: 'delta', tolerance: 0.02 },
        { name: 'human_review_ratio', min: 0.15, max: null, unit: 'ratio', tolerance: 0.03 },
        { name: 'decision_rate', min: 0, max: 5000, unit: 'valuations/hr', tolerance: 500 },
        { name: 'false_positive_rate', min: 0, max: 0.10, unit: 'rate', tolerance: 0.02 },
      ],
      geo: { type: 'logical', description: 'Approved MSAs and property types — residential, commercial, agricultural classifications' },
      time: { operating_hours: '00:00-23:59', operating_days: [0,1,2,3,4,5,6], timezone: 'ops_local' },
      states: { allowed: ['data_collection','valuation','confidence_check','appraiser_review','final'], forbidden: ['low_confidence_auto_approve','stale_comps_used','redlining_pattern_detected'] },
      odd_description: 'Automated property valuation for mortgage origination, refinancing, and portfolio assessment. High-variance or low-confidence valuations routed to licensed appraiser. ECOA/Fair Housing Act compliant with continuous bias monitoring.',
      safety: { violation_action: 'block', fail_closed: true, appraiser_escalation: true, fair_lending_compliance: true, audit_trail: true },
    },
  },
  dynamic_pricing_ai: {
    label: 'Dynamic Pricing AI (Regulated)',
    domain: 'financial',
    template: {
      numeric: [
        { name: 'max_price_multiplier', min: 1.0, max: 3.0, unit: 'x_base', tolerance: 0.1 },
        { name: 'max_price_change_rate', min: 0, max: 10, unit: '%/hr', tolerance: 1 },
        { name: 'min_price_floor', min: 0.01, max: null, unit: 'currency', tolerance: 0 },
        { name: 'bias_disparity_ratio', min: 0, max: 1.10, unit: 'ratio', tolerance: 0.02 },
        { name: 'confidence_threshold', min: 0.80, max: null, unit: 'score', tolerance: 0.03 },
        { name: 'decision_rate', min: 0, max: 10000, unit: 'price_updates/hr', tolerance: 1000 },
        { name: 'human_review_ratio', min: 0.01, max: null, unit: 'ratio', tolerance: 0.005 },
        { name: 'data_staleness_max', min: 0, max: 300, unit: 'sec', tolerance: 30 },
      ],
      geo: { type: 'logical', description: 'Service regions with applicable price gouging statutes and regulatory caps' },
      time: { operating_hours: '00:00-23:59', operating_days: [0,1,2,3,4,5,6], timezone: 'ops_local' },
      states: { allowed: ['normal_pricing','surge','promotional','regulatory_cap','manual_override','emergency_freeze'], forbidden: ['price_ceiling_exceeded','discriminatory_pricing_detected','emergency_gouging'] },
      odd_description: 'Real-time dynamic pricing within regulatory price caps and anti-gouging statutes. System enforces price ceilings during declared emergencies. Discriminatory pricing patterns trigger automatic freeze and human review.',
      safety: { violation_action: 'block', fail_closed: true, price_ceiling_enforcement: true, emergency_freeze: true, audit_trail: true },
    },
  },
